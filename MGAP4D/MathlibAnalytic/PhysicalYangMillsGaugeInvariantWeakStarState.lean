...
/-- The `n`-th normalized positive gauge-invariant state in the weak-star dual. -/
noncomputable def physicalYangMillsApproximatingGaugeInvariantWeakStarState
  ...
  StrongDual.toWeakDual
    (physicalYangMillsApproximatingGaugeInvariantContinuousState S n).toContinuousLinearMap

/-- The continuum normalized positive gauge-invariant state in the weak-star dual. -/
noncomputable def physicalYangMillsContinuumGaugeInvariantWeakStarState
    (S : PhysicalFourDimensionalYangMillsSymmetryLimit) :
    WeakDual ℝ (physicalYangMillsGaugeInvariantObservableSubalgebra S) :=
  StrongDual.toWeakDual
    (physicalYangMillsContinuumGaugeInvariantContinuousState S).toContinuousLinearMap
...
/-- The embedded-lattice states converge to the continuum state weak-star. -/
theorem physical_yang_mills_gaugeInvariantWeakStarState_converges
  ...
  intro O
  change Tendsto
    (fun n : ℕ => physicalYangMillsApproximatingGaugeInvariantWeakStarState S n O)
    atTop
