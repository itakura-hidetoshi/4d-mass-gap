import MGAP4D.MathlibAnalytic.FiniteOrientedLatticeWilsonCanonicalTotalVariationDefiniteness
...
/-- Scaling an oriented observable gives the expected upper bound on each
canonical physical-link variation. -/
theorem finite_oriented_canonicalLinkVariation_smul_le
  ...
  let P : FiniteOrientedLatticeWilsonLinkVariationBound L (c • f) :=
    { variation := fun source =>
        |c| * L.canonicalLinkVariation f source
      variation_nonneg := by
        intro source
        exact mul_nonneg (abs_nonneg c)
          (finite_oriented_canonicalLinkVariation_nonneg L f source)
      variation_bound := by
...
/-- Canonical physical-link variation is exactly homogeneous under real scalar
multiplication. -/
theorem finite_oriented_canonicalLinkVariation_smul
    (L : FiniteOrientedLatticeWilsonSystem)
    (c : ℝ)
    (f : L.Configuration → ℝ)
    (e : L.Edge) :
  ...
  · subst c
    ...
    exact finite_oriented_canonicalLinkVariation_const_eq_zero L 0 e
  · have hForward :=
      finite_oriented_canonicalLinkVariation_smul_le L c f e
    have hInv :
        L.canonicalLinkVariation f e ≤
          |c⁻¹| * L.canonicalLinkVariation (c • f) e := by
      simpa [smul_smul, hc] using
        (finite_oriented_canonicalLinkVariation_smul_le
          L c⁻¹ (c • f) e)
    have hBackward :
...
/-- Canonical oriented total variation is exactly homogeneous under real
scalar multiplication. -/
theorem finite_oriented_canonicalTotalVariation_smul
    (L : FiniteOrientedLatticeWilsonSystem)
    (c : ℝ)
    (f : L.Configuration → ℝ) :
  ...
  calc
    (∑ e : L.Edge, L.canonicalLinkVariation (c • f) e) =
      ...
      intro e _he
      exact finite_oriented_canonicalLinkVariation_smul L c f e
