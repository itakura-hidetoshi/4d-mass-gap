...
/-!
An operator-valued bounded Borel spectral integral is the remaining concrete
input needed to construct the ball-local inverse of the canonical restricted
Hamiltonian.  The package below keeps the integrand measurable and uniformly
bounded in its type, identifies indicator integrals with the existing PVM, and
records the standard graph law for multiplication by the shifted coordinate.
-/
...
The integral is an operator-valued map on bounded Borel functions.  Indicator
functions recover the ambient PVM after inclusion into the physical Hilbert
space.  The shifted-coordinate graph law states that whenever `g(t) =
(t-E)f(t)`, the vector `∫ f dP ψ` lies in the restricted Hamiltonian domain and
its real shift is `∫ g dP ψ`. -/
...
theorem explicit_wightman_os_exact_gap_pvm_open_support_of_boundedBorelSpectralIntegral
    ...
    ExplicitWightmanOSExactGapPVMOpenSupportProp M
      (((F.toBallFunctionalCalculus hUniform)
        .toCanonicalRestrictedPVMSpectralTheorem hSelf hSupport)
