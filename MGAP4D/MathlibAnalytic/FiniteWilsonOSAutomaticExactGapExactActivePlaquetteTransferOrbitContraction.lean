...
the refined exact off-diagonal plaquette profile at every Wilson scale.

The active profile removes the exact zero diagonal before taking the degree and
local-influence maxima.  Correlation representation and continuum convergence
remain explicit independent inputs. -/
structure
    FiniteWilsonOSAutomaticExactGapExactActivePlaquetteTransferOrbitContractionData
    (W : FiniteWilsonOSAutomaticApproximationFamily) where
  Observable : Type
  profileData : FiniteWilsonExactActivePlaquetteDobrushinFamilyData W
  gapScale : W.index
  scale : ℕ → W.index
  leftObservable :
...
/-- Enter the generic canonical-strict transfer theorem after generating the
strict family from the refined active plaquette profile. -/
noncomputable def
    FiniteWilsonOSAutomaticExactGapExactActivePlaquetteTransferOrbitContractionData.toStrictTransferData
...
/-- Refined active plaquette-profile strictness generates the finite-scale
transfer-orbit connected-correlation estimate. -/
theorem finite_wilson_exact_gap_bound_of_exact_active_plaquette_profile
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D :
      FiniteWilsonOSAutomaticExactGapExactActivePlaquetteTransferOrbitContractionData
        W)
...
/-- Under the explicit representation and pointwise convergence assumptions,
refined active plaquette-profile strictness yields the continuum exact-gap
connected-correlation decay estimate. -/
theorem finite_wilson_exact_gap_exact_active_plaquette_continuum_bound
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D :
      FiniteWilsonOSAutomaticExactGapExactActivePlaquetteTransferOrbitContractionData
        W)
