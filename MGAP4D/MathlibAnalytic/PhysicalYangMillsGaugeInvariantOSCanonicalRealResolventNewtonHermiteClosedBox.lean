...
/-- Canonical OS simultaneous convergence of Newton-Hermite interpolants and
their exact remainders on a complete closed half-mass Taylor box for arbitrary
joint admissible-time and Taylor-degree nets. -/
theorem VacuumSemigroupGapSlope.canonicalRescaledDefectTaylorPartialSum_realResolventNewtonHermitePair_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in m, ∀ p, box.Contains p → ∀ q ∈ C,
      ‖continuousLinearMapRealResolventNewtonHermitePair interpolationDegree
          (continuousLinearMapCompression J Q (continuousLinearMapTaylorPartialSum
            ...
            p.center p.target (degree b))) (nodes q) (eval q) -
        continuousLinearMapRealResolventNewtonHermitePair interpolationDegree
