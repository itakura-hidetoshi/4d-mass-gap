...
noncomputable def physicalStateLinearMap
  ...
  map_add' := by
    intro F G
    simpa [EuclideanYangMillsOSPositiveTimeObservableConstruction.physicalState,
      EuclideanYangMillsOSPositiveTimeObservableConstruction.osClass] using
      congrArg
        (fun q : P.OSSeparatedPreHilbert => (q : P.PhysicalHilbert))
