...
This is the key local separation fact used to evaluate a finite coordinate-unit
synthesis at a selected coordinate. -/
...
This is the first all-`Fin m` kernel/range theorem: injectivity of the selected
index map `φ : Fin m → ℕ` implies coordinate recovery, hence bottom kernel,
injectivity, bijective range map, range linear equivalence, and range coordinate
reconstruction. -/
