...
theorem iteratedDeriv_realResolventNewtonHermiteLinearResponseFamilyPair_finiteDimensionalCompression_tendsto_uniformOn_compact_product_rectangular
    ...
    (J : V →L[ℝ] E) (Q : E →L[ℝ] V) (taylorOrder interpolationOrder : ℕ) {observableOrder : ℕ}
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ a in l, ∀ k : Fin (taylorOrder + 1), ∀ d : Fin (interpolationOrder + 1), ∀ lambda ∈ K, ∀ q ∈ T,
      ‖continuousLinearMapRealResolventLinearResponseFamilyPair Phi (continuousLinearMapRealResolventNewtonHermitePair d.1 (continuousLinearMapCompression J Q (_root_.iteratedDeriv k.1 (F a) lambda)) (nodes d q) (eval d q)) - continuousLinearMapRealResolventLinearResponseFamilyPair Phi (continuousLinearMapRealResolventNewtonHermitePair d.1 (continuousLinearMapCompression J Q (_root_.iteratedDeriv k.1 S.limitResolvent lambda)) (nodes d q) (eval d q))‖ < epsilon := by
