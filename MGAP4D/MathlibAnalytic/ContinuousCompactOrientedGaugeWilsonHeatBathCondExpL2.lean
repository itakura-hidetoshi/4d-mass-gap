...
theorem continuous_compact_oriented_memLp_two_of_uniform_bound
  ...
  have hConst : MemLp (fun _ : C.base.Configuration => M) 2 C.gibbsMeasure := memLp_const M
...
theorem continuous_compact_oriented_singleLinkHeatBathProjection_abs_le
  ...
  calc
    |∫ B, f B ∂C.singleLinkHeatBathKernel target A| =
        ‖∫ B, f B ∂C.singleLinkHeatBathKernel target A‖ := by rw [Real.norm_eq_abs]
    _ ≤ ∫ B, ‖f B‖ ∂C.singleLinkHeatBathKernel target A := norm_integral_le_integral_norm _
...
theorem continuous_compact_oriented_singleLinkHeatBathProjection_ae_eq_condExpL2
    ...
    C.singleLinkHeatBathProjection target f =ᵐ[C.gibbsMeasure]
      (condExpL2 ℝ ℝ (compact_oriented_offLinkMeasurableSpace_le C.base target)
        ((continuous_compact_oriented_memLp_two_of_uniform_bound
          C f hf M hM0 hM).toLp f) : Lp ℝ 2 C.gibbsMeasure) := by
  letI : IsProbabilityMeasure C.gibbsMeasure :=
    continuous_compact_oriented_gibbsMeasure_isProbabilityMeasure C
  ...
  let hfLp : MemLp f 2 C.gibbsMeasure :=
    continuous_compact_oriented_memLp_two_of_uniform_bound C f hf M hM0 hM
  let Pf : C.base.Configuration → ℝ := C.singleLinkHeatBathProjection target f
  ...
  have hPfBound : ∀ A, |Pf A| ≤ M :=
    continuous_compact_oriented_singleLinkHeatBathProjection_abs_le C target f hf M hM0 hM
  ...
  have hpMeas : AEStronglyMeasurable[m] pL2 C.gibbsMeasure :=
    hPfStrong.aestronglyMeasurable.congr hPfLp.coeFn_toLp.symm
  have hcMeas : AEStronglyMeasurable[m]
      (condExpL2 ℝ ℝ hm fL2 : Lp ℝ 2 C.gibbsMeasure) C.gibbsMeasure :=
  ...
  have hLpEq : pL2 =ᵐ[C.gibbsMeasure]
    ...
    · intro s hs hμs
      exact integrableOn_Lp_of_measure_ne_top pL2 fact_one_le_two_ennreal.elim hμs.ne
    · intro s hs hμs
    ...
    · intro s hs hμs
      calc
        ∫ A in s, pL2 A ∂C.gibbsMeasure = ∫ A in s, Pf A ∂C.gibbsMeasure :=
          setIntegral_congr_ae (hm s hs) (ae_restrict_of_ae hPfLp.coeFn_toLp)
        ...
        _ = ∫ A in s, fL2 A ∂C.gibbsMeasure :=
          setIntegral_congr_ae (hm s hs) (ae_restrict_of_ae hfLp.coeFn_toLp.symm)
        _ = ∫ A in s, (condExpL2 ℝ ℝ hm fL2 : Lp ℝ 2 C.gibbsMeasure) A
            ∂C.gibbsMeasure := (integral_condExpL2_eq hm fL2 hs hμs.ne).symm
    · exact hpMeas
    · exact hcMeas
