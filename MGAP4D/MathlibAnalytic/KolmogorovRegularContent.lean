...
theorem regular_addContent_tendsto_zero
  ...
  have happ : ∀ n, ∃ K, K ∈ C ∧ K ⊆ s n ∧ m (s n \ K) ≤ δ n :=
    fun n => hreg (s n) (hs n) (δ n) (mod_cast hδpos n)
  choose t htC hts hmt using happ
  rw [eventually_atTop]
  have htInter : ⋂ n, t n = ∅ :=
    Set.subset_eq_empty (Set.iInter_mono hts) hsInter
  let S := hC.support htC htInter
  have hS := hC.iInter_eq_empty htC htInter
  have hSnonempty : Finset.Nonempty (Finset.Iic S) := by simp
  let N := Finset.max' (Finset.Iic S) hSnonempty
  have htEmpty : ∀ n, N ≤ n → ⋂ i ≤ n, t i = ∅ := by
    intro n hn
  ...
  calc
    ...
    _ = m ((⋂ i ≤ n, s i) \ (⋂ i ≤ n, t i)) := by
      simp only [htEmpty n hn, Set.diff_empty]
    _ ≤ m (⋃ i ≤ n, s i \ t i) := by
      ...
      · rw [Set.diff_iInter]
        ...
        · simp only [hin, Set.iInter_of_empty, Set.diff_univ,
            Set.iUnion_of_empty, Set.empty_subset]
