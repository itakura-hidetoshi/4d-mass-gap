...
theorem finiteEvenFourTorusZ2UnfixedGaugePerron_approximateTensorization
  ...
  letI : Nonempty (FiniteEvenFourTorusSpatialLink H) :=
    ⟨(⟨0, by simp⟩, ⟨1, by decide⟩)⟩
  have hCard :
  ...
  have hCenter :
      finitePositiveWeightSum
        (finiteEvenFourTorusZ2UnfixedGaugePerronDensity
          H β energyIdentity energyNontrivial hβ hEnergy)
        (fun A => f A) = 0 := by
    rw [← finiteEvenFourTorusZ2UnfixedGauge_weightedMean_eq_positiveWeightSum
...
theorem weightedDoobRayleigh
  ...
  have hCompare : V ≤ C.comparisonConstant * (N - Q) := by
    simpa [N, Q, V] using
      Z2UnfixedGaugePerronDobrushinDoobComparisonCertificate
        .totalSingleLinkVariance_le_doobDirichlet C H f
  have hCombined :
      C.comparisonConstant * Q ≤
        (C.comparisonConstant - C.margin) * N := by
    nlinarith
  have hComparisonPos : 0 < C.comparisonConstant :=
    Z2UnfixedGaugePerronDobrushinDoobComparisonCertificate
      .comparisonConstant_pos C
  calc
    Q ≤ ((C.comparisonConstant - C.margin) * N) /
        C.comparisonConstant :=
      (le_div_iff₀ hComparisonPos).2 hCombined
    _ = C.rayleighRate * N := by
      unfold rayleighRate
      field_simp [ne_of_gt hComparisonPos]
      ring
...
theorem rayleighRate_lt_one
    ...
    C.rayleighRate < 1 := by
  unfold rayleighRate
  have hMarginPos : 0 < C.margin :=
    Z2UnfixedGaugePerronDobrushinDoobComparisonCertificate.margin_pos C
  have hComparisonPos : 0 < C.comparisonConstant :=
    Z2UnfixedGaugePerronDobrushinDoobComparisonCertificate
      .comparisonConstant_pos C
  have hRatio : 0 < C.margin / C.comparisonConstant :=
    div_pos hMarginPos hComparisonPos
  linarith
