...
/-- Compact-uniform convergence of the complete Taylor-order by joint-Fréchet
carrier jet when the compressed resolvent and the entire finite perturbation
family move simultaneously. -/
theorem iteratedDeriv_realResolventJointMultilinearCarrierRectangularJet_finiteDimensionalCompression_tendsto_uniformOn_compact_product_directionFamily_sup
  ...
  let P : α → I → Prop := fun a kn => ∀ lambda ∈ K, ∀ z ∈ Z,
    ‖continuousLinearMapJointSpectralOperatorRealResolventMultilinearCarrierFromResolvent
        m kn.2.1 (H a) (continuousLinearMapCompressedRealResolventAt J Q
          (_root_.iteratedDeriv kn.1.1 (F a) lambda) z) -
      continuousLinearMapJointSpectralOperatorRealResolventMultilinearCarrierFromResolvent
        m kn.2.1 H0 (continuousLinearMapCompressedRealResolventAt J Q
  ...
  have hkn : ∀ kn : I, ∀ᶠ a in l, P a kn := by
    ...
    have hout :=
      finiteDimensional_directionFamilyObservable_tendsto_uniformOn
        R R0 H H0
        (fun p =>
          continuousLinearMapJointSpectralOperatorRealResolventMultilinearCarrierFromResolvent
            m kn.2.1 p.2 p.1)
...
/-- Compact-uniform convergence of the complete Banach-valued response jet
under simultaneous resolvent and direction-family motion. -/
theorem iteratedDeriv_realResolventJointMultilinearResponseCarrierRectangularJet_finiteDimensionalCompression_tendsto_uniformOn_compact_product_directionFamily_sup
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    (S : ContinuousLinearMapOpenTaylorStrongLimitData l gap F)
    (B : ContinuousLinearMapOpenResolventNormBoundFamilyData gap F)
    (L : ContinuousLinearMapOpenResolventNormBoundData E)
  ...
  let P : α → I → Prop := fun a kn => ∀ lambda ∈ K, ∀ z ∈ Z,
    ‖continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierFromResolvent
        φ m kn.2.1 (H a) (continuousLinearMapCompressedRealResolventAt J Q
          (_root_.iteratedDeriv kn.1.1 (F a) lambda) z) -
      continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierFromResolvent
        φ m kn.2.1 H0 (continuousLinearMapCompressedRealResolventAt J Q
          (_root_.iteratedDeriv kn.1.1 S.limitResolvent lambda) z)‖ < epsilon
  have hkn : ∀ kn : I, ∀ᶠ a in l, P a kn := by
    intro kn
    let R : α → (ℝ × ℝ) → (V →L[ℝ] V) := fun a p =>
    ...
    have hout :=
      finiteDimensional_directionFamilyObservable_tendsto_uniformOn
        ...
        (continuous_continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierCompleteJetFromResolvent_directionFamily
          V m (kn.2.1 + 1)).apply_continuous kn.2
        M hM hR0 hR hH epsilon hepsilon
    filter_upwards [hout] with a ha
    intro lambda hlambda z hz
    simpa [P, R, R0,
      continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierCompleteJetFromResolvent] using
