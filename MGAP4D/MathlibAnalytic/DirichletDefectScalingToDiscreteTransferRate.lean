...
theorem lowerRate_le_massRate
  ...
  have hnum : defect n / 2 ≤ -Real.log (Real.sqrt x) := by
    ...
    linarith
  have hdiv :
      (defect n / 2) / latticeSpacing n ≤
        (-Real.log (Real.sqrt x)) / latticeSpacing n :=
    (div_le_div_iff_of_pos_right (A.latticeSpacing_pos n)).2 hnum
  unfold lowerRate massRate defectRate transferFactor
  dsimp [x] at hdiv ⊢
...
theorem massRate_le_upperRate
  ...
  have hinv : x⁻¹ - 1 = defect n * x⁻¹ := by
    have haux : x⁻¹ - 1 = (1 - x) * x⁻¹ := by
      field_simp [hxne]
      <;> ring
    have hxdef : 1 - x = defect n := by
      dsimp [x]
      ring
    calc
      x⁻¹ - 1 = (1 - x) * x⁻¹ := haux
      _ = defect n * x⁻¹ := by rw [hxdef]
  ...
  have hdiv :
      (-Real.log (Real.sqrt x)) / latticeSpacing n ≤
        ((defect n * x⁻¹) / 2) / latticeSpacing n :=
    (div_le_div_iff_of_pos_right (A.latticeSpacing_pos n)).2 hnum
  unfold massRate upperRate defectRate transferFactor
  dsimp [x] at hdiv ⊢
  convert hdiv using 1 <;> ring
