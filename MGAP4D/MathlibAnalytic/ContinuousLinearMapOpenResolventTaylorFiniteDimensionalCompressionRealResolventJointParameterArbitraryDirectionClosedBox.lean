...
/-- Closed-box convergence of an arbitrary Banach-valued observation of a
fixed genuine joint Fréchet derivative for arbitrary joint nets. -/
theorem taylorPartialSum_realResolventJointArbitraryDirectionResponse_tendsto_uniform_closedBox_of_joint
    ...
    (u : Fin n → (Fin (m + 1) → ℝ)) {q : Filter β}
    (a : β → α) (degree : β → ℕ) (ha : Tendsto a q l)
    (hdegree : Tendsto degree q atTop)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in q, ∀ p, box.Contains p → ∀ z ∈ Z,
      ‖continuousLinearMapJointSpectralOperatorRealResolventSymmetricDerivativeLinearResponse
          φ m n (continuousLinearMapCompression J Q
            (continuousLinearMapTaylorPartialSum (F (a b)) p.center p.target (degree b)))
          H z 0 0 u -
        continuousLinearMapJointSpectralOperatorRealResolventSymmetricDerivativeLinearResponse
...
/-- Simultaneous closed-box convergence of the complete finite
arbitrary-direction joint Fréchet response jet. -/
theorem taylorPartialSum_realResolventJointArbitraryDirectionResponse_tendsto_uniform_closedBox_of_joint_rectangular
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    (S : ContinuousLinearMapOpenTaylorStrongLimitData l gap F)
    (B : ContinuousLinearMapOpenResolventNormBoundFamilyData gap F)
    (L : ContinuousLinearMapOpenResolventNormBoundData E)
    ...
    {q : Filter β} (a : β → α) (degree : β → ℕ) (ha : Tendsto a q l)
    (hdegree : Tendsto degree q atTop)
    (box : ContinuousLinearMapClosedTaylorParameterBox gap) (Z : Set ℝ)
    (margin : ℝ) (hmargin : 0 < margin)
    (hlimitMargin : ∀ p, box.Contains p → ∀ z ∈ Z, margin ≤
    ...
    (hlimitNorm : ∀ p, box.Contains p → ∀ z ∈ Z,
      continuousLinearMapRealResolventNorm
        (continuousLinearMapCompression J Q (S.limitResolvent p.target)) z ≤ M) :
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in q, ∀ n : Fin (mixedOrder + 1),
  ...
  simpa [continuousLinearMapJointSpectralOperatorRealResolventSymmetricDerivativeLinearResponse,
    ...
    S.taylorPartialSum_realResolventFiniteParameterMixedResponse_tendsto_uniform_closedBox_of_joint_rectangular
      B L hLgap hLresolvent J Q φ (m + 1) mixedOrder
      (continuousLinearMapJointSpectralOperatorDirectionFamily m H) u
      a degree ha hdegree box Z margin hmargin hlimitMargin M hM hlimitNorm

/-- Closed-box convergence of the basis-independent trace of a fixed genuine
joint Fréchet derivative in arbitrary directions. -/
theorem taylorPartialSum_realResolventJointArbitraryDirectionTrace_tendsto_uniform_closedBox_of_joint
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    (S : ContinuousLinearMapOpenTaylorStrongLimitData l gap F)
    (B : ContinuousLinearMapOpenResolventNormBoundFamilyData gap F)
    (L : ContinuousLinearMapOpenResolventNormBoundData E)
  ...
  simpa [continuousLinearMapJointSpectralOperatorRealResolventSymmetricTraceDerivative,
    ...
    S.taylorPartialSum_realResolventJointArbitraryDirectionResponse_tendsto_uniform_closedBox_of_joint
      ...
      m n H u a degree ha hdegree box Z margin hmargin hlimitMargin M hM hlimitNorm
