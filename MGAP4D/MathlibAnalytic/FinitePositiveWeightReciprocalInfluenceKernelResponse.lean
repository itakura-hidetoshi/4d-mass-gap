...
/-- A weight-independent nonnegative influence kernel. -/
structure FiniteNonnegativeInfluenceKernelData
    (ι : Type)
    [DecidableEq ι]
    [Fintype ι] where
  influence : ι → ι → ℝ
  influence_nonneg : ∀ target source : ι, 0 ≤ influence target source
...
/-- The universal full-`L¹` top kernel. -/
...
/-- Every canonical influence matrix is below the universal two kernel. -/
...
/-- One target update of a profile by a kernel. -/
...
/-- Kernel target updates preserve nonnegativity. -/
theorem finiteInfluenceKernelUpdatedVariation_nonneg
    ...
    (target source : ι) :
    0 ≤ finiteInfluenceKernelUpdatedVariation K variation target source := by
  ...
  · simp only [finiteInfluenceKernelUpdatedVariation, hEq, if_false]
    exact add_nonneg (hVariation source)
      (mul_nonneg (K.influence_nonneg target source) (hVariation target))

/-- Uniform random-scan average of kernel target updates. -/
...
/-- Reciprocal random-scan rate generated by a column coefficient. -/
...
/-- Nonnegativity of the reciprocal rate. -/
...
/-- Strictness of the reciprocal rate. -/
...
/-- Exact reciprocal-rate complement identity. -/
...
/-- A uniform column bound contracts the supremum profile. -/
  ...
  have hWeighted :
    ...
    calc
      ...
      _ ≤ columnCoefficient * bound :=
        mul_le_mul_of_nonneg_right (hColumnSum source) hBoundNonneg
  have hBase :
      n * variation source - variation source ≤ (n - 1) * bound := by
...
theorem finiteInfluenceKernelRandomScanVariationIterate_le_rate_pow_mul
    ...
    (n : ℕ)
    (source : ι) :
    finiteInfluenceKernelRandomScanVariationIterate K variation n source ≤
      finiteInfluenceKernelReciprocalRandomScanRate
...
/-- A concrete non-strict update is bounded by an entrywise larger kernel. -/
theorem finitePositiveWeightNonstrictRandomScanUpdatedVariation_le_kernel
    ...
    (hVariation : ∀ e : ι, 0 ≤ variation e)
    (source : ι) :
    finitePositiveWeightNonstrictRandomScanUpdatedVariation D variation source ≤
...
/-- The stationary comparison variation component is the profile iterate. -/
theorem
...
/-- Kernel updating distributes over finite sums. -/
theorem finiteInfluenceKernelUpdatedVariation_sum
    ...
    finiteInfluenceKernelUpdatedVariation
      ...
      ∑ k : κ,
        finiteInfluenceKernelUpdatedVariation K (family k) target source := by
  by_cases hEq : source = target
  · simp [finiteInfluenceKernelUpdatedVariation, hEq]
  · simp only [finiteInfluenceKernelUpdatedVariation, hEq, if_false]
    rw [Finset.sum_add_distrib, Finset.mul_sum]

/-- Random-scan updating distributes over finite sums. -/
theorem finiteInfluenceKernelRandomScanUpdatedVariation_sum
    ...
    finiteInfluenceKernelRandomScanUpdatedVariation
      ...
      ∑ k : κ,
        finiteInfluenceKernelRandomScanUpdatedVariation K (family k) source := by
  unfold finiteInfluenceKernelRandomScanUpdatedVariation
  simp_rw [finiteInfluenceKernelUpdatedVariation_sum]
  rw [Finset.sum_comm, Finset.mul_sum]

/-- Every finite kernel iterate distributes over finite sums. -/
...
/-- Superposing singleton profiles gives the constant profile. -/
theorem finiteInfluenceKernelSingletonVariation_sum_source
    ...
    (coordinate : ι) :
    (∑ source : ι,
      finiteInfluenceKernelSingletonVariation magnitude source coordinate) =
      magnitude := by
  classical
...
/-- Source-superposition of iterated singleton profiles. -/
...
/-- Reciprocal contraction of complete singleton superposition. -/
...
/-- Source-summed terminal total variation. -/
theorem finiteInfluenceKernelSingletonVariation_iterate_total_sum_source_le
  ...
  calc
    (∑ coordinate : ι,
      ...
      apply Finset.sum_le_sum
      intro coordinate _
      exact finiteInfluenceKernelSingletonVariation_iterate_sum_source_le
        K hCard columnCoefficient hColumnNonneg hColumnSum
...
/-- Kernel accumulated source response. -/
...
/-- Concrete accumulated response is bounded by the kernel response. -/
theorem
    ...
    C.partialStationarySource P n ≤
      finiteInfluenceKernelPartialSource K sourceEnvelope P.variation n := by
  induction n with
  | zero => exact le_rfl
  | succ n ih =>
      have hActualNonneg :
      ...
      have hProfile :
        ...
        exact finitePositiveWeightNonstrictRandomScanVariationIterate_le_kernel
...
/-- Finite geometric series bounded by the resolvent. -/
...
/-- Cancellation of card normalization in the reciprocal resolvent. -/
