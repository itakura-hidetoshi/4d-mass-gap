...
/-- Uniform convergence of a fixed mixed-direction Dyson polarization
coefficient on a complete closed Taylor box for arbitrary joint time and
Taylor-degree nets. -/
theorem taylorPartialSum_realResolventOperatorMixedDysonCoefficient_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint
  ...
  simpa [R, R0,
    continuousLinearMapRealResolventOperatorMixedDysonCoefficient_eq_fromResolvent] using hb'

/-- Simultaneous closed-box convergence of the full finite mixed-direction
polarization jet for arbitrary joint time and Taylor-degree nets. -/
theorem taylorPartialSum_realResolventOperatorMixedDysonCoefficient_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint_rectangular
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    (S : ContinuousLinearMapOpenTaylorStrongLimitData l gap F)
    (B : ContinuousLinearMapOpenResolventNormBoundFamilyData gap F)
    (L : ContinuousLinearMapOpenResolventNormBoundData E)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in m,
      ∀ n : Fin (mixedOrder + 1), ∀ p, box.Contains p → ∀ z ∈ Z,
        ‖continuousLinearMapRealResolventOperatorMixedDysonCoefficient n.1
            (continuousLinearMapCompression J Q
              (continuousLinearMapTaylorPartialSum
                (F (a b)) p.center p.target (taylorDegree b))) (H n) z -
          continuousLinearMapRealResolventOperatorMixedDysonCoefficient n.1
      ...
      ‖continuousLinearMapRealResolventOperatorMixedDysonCoefficient n.1
          (continuousLinearMapCompression J Q
            (continuousLinearMapTaylorPartialSum
              (F (a b)) p.center p.target (taylorDegree b))) (H n) z -
        continuousLinearMapRealResolventOperatorMixedDysonCoefficient n.1
          (continuousLinearMapCompression J Q (S.limitResolvent p.target)) (H n) z‖ < epsilon
  have hn : ∀ n : Fin (mixedOrder + 1), ∀ᶠ b in m, P b n := by
    intro n
    exact
  ...
  have hfinite : ∀ᶠ b in m, ∀ n : Fin (mixedOrder + 1), P b n := by
    change {b | ∀ n : Fin (mixedOrder + 1), P b n} ∈ m
    rw [show {b | ∀ n : Fin (mixedOrder + 1), P b n} =
      ⋂ n ∈ (Finset.univ : Finset (Fin (mixedOrder + 1))), {b | P b n} by
        ext b
        simp]
    exact (Filter.biInter_finset_mem
      (Finset.univ : Finset (Fin (mixedOrder + 1)))).2
      (fun n _ => hn n)
