...
import MGAP4D.MathlibAnalytic.R4HilbertMathlibSelfAdjointOperatorBareBoundedActualRouteFromFamily
...
/-! Completed-OS-carrier coverage to bare bounded actual route. -/

/-- Completed-OS-carrier coverage supplies the bare bounded actual route witness. -/
def r4HilbertMathlibSelfAdjointOperator_bare_bounded_actual_route_from_completed_os_carrier
  ...
  r4HilbertMathlibSelfAdjointOperator_bare_bounded_actual_route_from_bounded_route_family I TR
    (r4HilbertMathlibSelfAdjointOperator_bounded_route_family_from_completed_os_carrier I TR
      hBridge oscover)

/-- Completed-OS-carrier coverage gives bounded actual data for bare `M`. -/
def r4HilbertMathlibSelfAdjointOperator_completed_os_carrier_to_bare_M_bounded_actual_data
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (oscover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorCompletedOSCarrierCoverageData I TR M)
...
/-- Completed-OS-carrier coverage gives nonempty bounded actual data for bare `M`. -/
theorem r4HilbertMathlibSelfAdjointOperator_completed_os_carrier_to_bare_M_bounded_actual_nonempty
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (oscover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorCompletedOSCarrierCoverageData I TR M)
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) :
    Nonempty (R4HilbertMathlibSelfAdjointOperatorBoundedActualData I TR M) := by
  exact ⟨r4HilbertMathlibSelfAdjointOperator_completed_os_carrier_to_bare_M_bounded_actual_data I TR
    hBridge oscover M⟩

/-- Completed-OS-carrier coverage gives a continuous top-domain representative. -/
theorem r4HilbertMathlibSelfAdjointOperator_completed_os_carrier_to_bare_M_top_representative
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (oscover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorCompletedOSCarrierCoverageData I TR M)
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) :
