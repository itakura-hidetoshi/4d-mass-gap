...
theorem VacuumSemigroupGapSlope.canonicalFiniteParameterMixedResponse_tendsto_uniform_closedBox_of_joint
    ...
    (φ : (V →L[ℝ] V) →L[ℝ] W) (parameterDimension mixedOrder : ℕ)
...
theorem VacuumSemigroupGapSlope.canonicalFiniteParameterTaylorResponse_tendsto_uniform_closedBox_of_joint
    ...
    (hSelf : IsSelfAdjoint T.closedRightHamiltonian)
    (J : V →L[ℝ] P.VacuumOrthogonalHilbert) (Q : P.VacuumOrthogonalHilbert →L[ℝ] V)
    (φ : (V →L[ℝ] V) →L[ℝ] W) (parameterOrder parameterDimension : ℕ)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in m, ∀ p, box.Contains p → ∀ z ∈ Z,
      ‖continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse φ parameterOrder parameterDimension
        ...
        continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse φ parameterOrder parameterDimension
          (continuousLinearMapCompression J Q
            (G.vacuumOrthogonalContinuumTaylorResolvent T hP hInnerSymmetric hSelf p.target)) H z 0 h‖ < epsilon := by
  exact
    (G.canonicalRescaledDefectTaylorStrongLimitData
...
theorem VacuumSemigroupGapSlope.canonicalFiniteParameterTaylorTrace_tendsto_uniform_closedBox_of_joint
    ...
    (h : Fin parameterDimension → ℝ) {β : Type*} {m : Filter β}
