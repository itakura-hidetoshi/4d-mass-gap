...
/-- Spectral-analysis surface for deriving the normalized value `33/20` from the
Yang--Mills continuum Hamiltonian chain.

This is an additive theorem surface.  It does not replace the upstream
continuum-Hamiltonian witness, and it does not widen the public-release or
external-consensus boundary.  Its role is to make the direction explicit:

* the continuum Yang--Mills lane builds the physical Hamiltonian witness;
* the self-adjoint/spectral chain is ready;
* the Rayleigh lower-bound surface supplies the spectral infimum side;
* the Rayleigh attainment surface supplies the eigenvalue/attainment side;
* the positive spectral-mass surface supplies the observable spectral atom;
* these spectral facts force the normalized Hamiltonian spectral value to be
  `33/20`. -/
...
/-- Ready predicate for the Yang--Mills Hamiltonian spectral derivation of
`33/20`. -/
...
/-- Installed spectral-derivation surface. -/
...
/-- The installed Yang--Mills Hamiltonian spectral derivation is ready. -/
...
/-- The Yang--Mills Hamiltonian spectral infimum side gives `33/20`. -/
