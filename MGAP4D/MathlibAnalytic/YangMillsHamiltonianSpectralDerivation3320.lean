...
/-- The Yang--Mills Hamiltonian spectral carrier evaluates to `33 / 20` only
after the Hamiltonian/spectral carrier has been identified with the normalized
exact-gap carrier.  Thus the numeric value is reached through the spectral route,
not introduced as a detached downstream constant. -/
theorem yang_mills_hamiltonian_spectral_value_eq_33_over_20 :
  ...
  have hCarrier : exactGapValueReal = (33 : ℝ) / 20 := by
...
/-- Typed proof surface for the intended physical reading:

Yang--Mills dynamics builds the continuum Hamiltonian, the Hamiltonian enters the
self-adjoint spectral theorem/PVM lane, the spectral infimum, attained spectral
value, and observable PVM atom are all forced to the same Hamiltonian spectral
carrier, that atom has positive nonzero spectral mass, and the carrier therefore
has the normalized gap value `33 / 20`. -/
def yangMillsHamiltonianSpectralPVMDerivesExactGap3320 : Prop :=
...
/-- Yang--Mills Hamiltonian analysis, through the spectral theorem/PVM atom lane,
forces the exact normalized gap value. -/
theorem yang_mills_hamiltonian_spectral_pvm_derives_exact_gap_3320 :
...
theorem yang_mills_hamiltonian_spectral_theorem_pvm_forces_exact_gap_33_over_20 :
    exactGapValueReal = (33 : ℝ) / 20 := by
  rcases yang_mills_hamiltonian_spectral_pvm_derives_exact_gap_3320 with
    ⟨_, _, _, _, _, _, _, _, _, hExact, _, _⟩
  exact hExact
