...
private theorem continuousVacuumPointwiseHarnack_integrable_mul_of_norm_le_one
    {α : Type*}
    [MeasurableSpace α]
    {μ : Measure α}
    {f k : α → ℝ}
  ...
  apply hf.mono' (hf.aestronglyMeasurable.mul hk)
...
private theorem continuousVacuumPointwiseHarnack_kernelSection_aestronglyMeasurable
    (H N : ℕ)
    (beta : ℝ)
...
private theorem continuousVacuumPointwiseHarnack_kernelSection_norm_le_one
    (H N : ℕ)
    (hN : 0 < N)
    (beta : ℝ)
    (hbeta : 0 ≤ beta)
...
values are taken only on the new continuous synthesis side; the old quotient
representative occurs solely under Haar integration. -/
...
/-- The unscaled continuous physical-vacuum synthesis satisfies the same
volume-independent one-link Harnack comparison as the one-slab Wilson kernel. -/
theorem periodicHypercubicEvenSpecialUnitaryPhysicalOneSlabVacuumSynthesisFunction_continuousVacuumReplaceLink_le_exp_eight_mul
    (H N : ℕ)
    (hN : 0 < N)
    (beta : ℝ)
    (hbeta : 0 ≤ beta)
  ...
  have hIg : Integrable
      (fun A => f A *
        periodicHypercubicEvenSpecialUnitaryTemporalGaugeOneSlabKernel
          H N beta A Bg) μ := by
    apply continuousVacuumPointwiseHarnack_integrable_mul_of_norm_le_one hfInt
    · simpa [μ, Bg] using
        continuousVacuumPointwiseHarnack_kernelSection_aestronglyMeasurable
          H N beta Bg
    · filter_upwards with A
      exact continuousVacuumPointwiseHarnack_kernelSection_norm_le_one
        H N hN beta hbeta Bg A
  have hIh : Integrable
      (fun A => f A *
        periodicHypercubicEvenSpecialUnitaryTemporalGaugeOneSlabKernel
          H N beta A Bh) μ := by
...
physical Wilson vacuum. The old `L²` quotient vacuum is never evaluated at a
prescribed fiber point. -/
theorem periodicHypercubicEvenSpecialUnitaryPhysicalOneSlabContinuousVacuumRepresentative_continuousVacuumReplaceLink_le_exp_eight_mul
  ...
  let lambda : ℝ :=
    ‖periodicHypercubicEvenSpecialUnitaryPhysicalOneSlabTransferOperator
      H N hN beta hbeta‖
  have hS :=
    periodicHypercubicEvenSpecialUnitaryPhysicalOneSlabVacuumSynthesisFunction_continuousVacuumReplaceLink_le_exp_eight_mul
  ...
  have hscaled := mul_le_mul_of_nonneg_left hS hlambdaInv
  simpa [lambda, mul_assoc, mul_left_comm, mul_comm] using hscaled
