import MGAP4D.MathlibAnalytic.R4HilbertMathlibSelfAdjointOperatorBareBoundedActualRoute
...
/-! Bridge from the unconditional bounded route to the bare-`M` route class. -/
...
/-- With an unconditional bounded route installed, the public bare-`M` data entry
point is available with only `M` explicit. -/
theorem r4HilbertMathlibSelfAdjointOperator_unconditional_to_bare_M_bounded_actual_nonempty
    (hRoute : r4HilbertMathlibSelfAdjointOperatorUnconditionalBoundedRoute I TR)
  ...
  letI : R4HilbertMathlibSelfAdjointOperatorBareBoundedActualRoute I TR :=
    r4HilbertMathlibSelfAdjointOperator_bare_bounded_actual_route_from_unconditional I TR hRoute
