...
/-- Canonical OS compact-uniform convergence of a fixed symmetric multilinear
resolvent derivative after arbitrary finite-dimensional compression. -/
theorem VacuumSemigroupGapSlope.canonicalRescaledDefectTaylorIteratedDeriv_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniformOn_compact_product
    ...
    (k symmetricOrder : ℕ) (H : Fin symmetricOrder → (V →L[ℝ] V)) (K : Set ℝ)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ tau in G.admissibleRescaledDefectTimeFilter,
      ∀ lambda ∈ K, ∀ z ∈ Z,
        ‖continuousLinearMapRealResolventOperatorSymmetricDerivative symmetricOrder
            (continuousLinearMapCompression J Q (_root_.iteratedDeriv k
              (G.admissibleRescaledDefectTaylorResolvent T hInnerSymmetric tau) lambda)) z H -
          continuousLinearMapRealResolventOperatorSymmetricDerivative symmetricOrder
            (continuousLinearMapCompression J Q (_root_.iteratedDeriv k
              (G.vacuumOrthogonalContinuumTaylorResolvent T hP hInnerSymmetric hSelf) lambda)) z H‖ < epsilon := by
  exact
    (G.canonicalRescaledDefectTaylorStrongLimitData
      T hP hInnerSymmetric hSelf).iteratedDeriv_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniformOn_compact_product
      ...
      rfl rfl J Q k symmetricOrder H K hKcompact hKu hu Z
      margin hmargin hlimitMargin M hM hlimitNorm

/-- Canonical OS simultaneous convergence on the finite Taylor-order by
finite symmetric multilinear derivative-order rectangle. -/
theorem VacuumSemigroupGapSlope.canonicalRescaledDefectTaylorIteratedDeriv_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniformOn_compact_product_rectangular
    (T : P.StronglyContinuousPhysicalSemigroup) (G : T.VacuumSemigroupGapSlope)
    (hP : P.IsNormalized) (hInnerSymmetric : T.toPhysicalSemigroup.IsInnerSymmetric)
    (hSelf : IsSelfAdjoint T.closedRightHamiltonian)
    (J : V →L[ℝ] P.VacuumOrthogonalHilbert) (Q : P.VacuumOrthogonalHilbert →L[ℝ] V)
    (taylorOrder symmetricOrder : ℕ)
    (H : ∀ n : Fin (symmetricOrder + 1), Fin n.1 → (V →L[ℝ] V)) (K : Set ℝ)
    (hKcompact : IsCompact K) {u : ℝ} (hKu : K ⊆ Set.Iic u)
    (hu : u < G.mass / 2) (Z : Set ℝ) (margin : ℝ) (hmargin : 0 < margin)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ tau in G.admissibleRescaledDefectTimeFilter,
      ∀ k : Fin (taylorOrder + 1), ∀ n : Fin (symmetricOrder + 1), ∀ lambda ∈ K, ∀ z ∈ Z,
        ‖continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
            (continuousLinearMapCompression J Q (_root_.iteratedDeriv k.1
              (G.admissibleRescaledDefectTaylorResolvent T hInnerSymmetric tau) lambda)) z (H n) -
          continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
            (continuousLinearMapCompression J Q (_root_.iteratedDeriv k.1
              (G.vacuumOrthogonalContinuumTaylorResolvent T hP hInnerSymmetric hSelf) lambda)) z (H n)‖ < epsilon := by
  exact
    (G.canonicalRescaledDefectTaylorStrongLimitData
      T hP hInnerSymmetric hSelf).iteratedDeriv_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniformOn_compact_product_rectangular
      (G.admissibleRescaledDefectOpenResolventNormBoundFamilyData T hInnerSymmetric)
      (G.vacuumOrthogonalContinuumOpenResolventNormBoundData T hP hInnerSymmetric hSelf)
      rfl rfl J Q taylorOrder symmetricOrder H K hKcompact hKu hu Z
      margin hmargin hlimitMargin M hM hlimitNorm
