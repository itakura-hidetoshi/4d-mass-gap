...
/-- Each selected quotient-section representative lies in the selected section range. -/
theorem quotientSection_mem_selectedRange
    ...
    (q : quotientCarrier I) :
    quotientSection I q ∈ X.sectionRange := by
  have hV : quotientSection I q ∈ V.sectionRange := V.sectionMemRange q
  simpa [Y.sectionRange_eq] using hV

/-- Selected quotient-section representatives are stable under quotient map followed by quotient section. -/
theorem quotientSection_representative_roundTrip
    (I : EuclideanYangMillsR4ReflectionPositiveReconstructionInputClosure S K R4 A G H N F C)
    {O : EuclideanYangMillsR4HilbertReconstructionCarrierClosure S K R4 A G H N F C I}
    {Q : EuclideanYangMillsR4HilbertReconstructionQuotientClosure S K R4 A G H N F C I O}
    {P : EuclideanYangMillsR4HilbertReconstructionQuotientProjectionClosure S K R4 A G H N F C I O Q}
    ...
    (Y : EuclideanYangMillsR4HilbertReconstructionQuotientSectionRangeRoundTripClosure S K R4 A G H N F C I O Q P R U J V W X)
    (q : quotientCarrier I) :
    quotientSection I (quotientMap I (quotientSection I q)) = quotientSection I q := by
  exact Y.sectionRangeRoundTrip (quotientSection_mem_selectedRange I Y q)

end EuclideanYangMillsR4HilbertReconstructionQuotient

/-- Model recording stability of selected quotient-section representatives.

This is still a carrier-level transport layer.  It packages that every selected
representative belongs to the selected section range and is stable under
`quotientSection ∘ quotientMap`. -/
structure EuclideanYangMillsR4HilbertReconstructionQuotientSectionRepresentativeStableModel
...
/-- Build the selected representative stability model from the section-range round-trip closure. -/
def ofSectionRangeRoundTripClosure
  ...
  { sectionRangeRoundTripClosure := Y
    sectionRangeRoundTripClosure_eq := rfl
    sectionRepresentativeMemRange :=
      EuclideanYangMillsR4HilbertReconstructionQuotient.quotientSection_mem_selectedRange I Y
    sectionRepresentativeRoundTrip :=
      EuclideanYangMillsR4HilbertReconstructionQuotient.quotientSection_representative_roundTrip I Y
    sectionRangeRoundTrip := Y.sectionRangeRoundTrip
    sectionRange_eq := Y.sectionRange_eq
    reflectionPositive := Y.reflectionPositive
    euclideanInvariant := Y.euclideanInvariant
    gaugeInvariant := Y.gaugeInvariant }
