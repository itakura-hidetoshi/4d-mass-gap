  ...
  have hproj0 :
      (∫ x,
        (WithLp.sndL 2 ℝ C₁.FeatureHilbert C₂.FeatureHilbert)
          (a x • (RealHilbertKernelFeature.add C₁ C₂).feature x) ∂μ) = 0 := by
    calc
      _ = (WithLp.sndL 2 ℝ C₁.FeatureHilbert C₂.FeatureHilbert)
          (∫ x, a x • (RealHilbertKernelFeature.add C₁ C₂).feature x ∂μ) := hproj
