...
`RayleighLowerBoundRealSurface`: the exact value `33/20` is itself an
...
/-- Any real Rayleigh energy attaining the exact gap has value `33/20`. -/
theorem rayleigh_attainment_energy_eq_3320
    (energy : ℝ) (henergy : RayleighAttainsExactGap energy) :
    energy = (33 : ℝ) / 20 := by
  exact henergy.2.trans exactGapValueReal_eq
...
theorem rayleigh_attainment_real_surface_value :
    rayleighAttainmentRealSurface.value = (33 : ℝ) / 20 := by
