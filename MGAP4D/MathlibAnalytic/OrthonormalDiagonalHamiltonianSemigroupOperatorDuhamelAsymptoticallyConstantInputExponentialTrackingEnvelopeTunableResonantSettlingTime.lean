...
/-- The tunable resonant left envelope yields an explicit logarithmic tracking time
at every target rate `0 < ν < δ`. -/
theorem orthonormalDiagonalHamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_tunable_resonant_left
    {ι E : Type*}
  ...
  let K := ‖A - S‖ + C / (Real.exp 1 * (δ - ν))
  have hgap : 0 < δ - ν := sub_pos.mpr hνδ
  have hdenpos : 0 < Real.exp 1 * (δ - ν) := mul_pos (Real.exp_pos _) hgap
  ...
  have henv :=
    orthonormalDiagonalHamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_exponentialTrackingEnvelope_tunable_resonant_left
      b a δ ν hδ hδpos hνpos hνδ t₀ t ht₀ A F U F_lim C hC hF
      (by
        intro s hs
        exact hFC s hs.1)
      hU0 hU
  ...
  · have hKpos : 0 < K := lt_of_le_of_ne hKnonneg (Ne.symm hK)
    have hwait_le : max 0 (Real.log (K / ε) / ν) ≤ t - t₀ := by
      simpa [S, K] using show max 0
        (Real.log
          ((‖A - orthonormalDiagonalHamiltonian_leftSteadyState b a F_lim‖ +
              C / (Real.exp 1 * (δ - ν))) / ε) / ν) ≤ t - t₀ from by
            linarith
...
/-- The tunable resonant right envelope has the identical explicit tracking time. -/
theorem orthonormalDiagonalHamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_tunable_resonant_right
    {ι E : Type*}
    [Fintype ι] [NormedAddCommGroup E] [InnerProductSpace ℝ E] [FiniteDimensional ℝ E]
    (b : OrthonormalBasis ι ℝ E) (a : ι → ℝ) (δ ν : ℝ)
    (hδ : ∀ i : ι, δ ≤ a i) (hδpos : 0 < δ)
