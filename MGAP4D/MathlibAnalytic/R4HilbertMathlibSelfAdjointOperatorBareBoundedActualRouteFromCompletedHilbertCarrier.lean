import MGAP4D.MathlibAnalytic.R4HilbertMathlibSelfAdjointOperatorBareBoundedActualRouteFromCompletedOSCarrier
...
/-! Completed-Hilbert-carrier coverage to bare bounded actual route. -/

/-- Completed-Hilbert-carrier coverage supplies the bare bounded actual route witness. -/
def r4HilbertMathlibSelfAdjointOperator_bare_bounded_actual_route_from_completed_hilbert_carrier
...
/-- Completed-Hilbert-carrier coverage gives bounded actual data for bare `M`. -/
def r4HilbertMathlibSelfAdjointOperator_completed_hilbert_carrier_to_bare_M_bounded_actual_data
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (hcover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorCompletedHilbertCarrierCoverageData I TR M)
...
/-- Completed-Hilbert-carrier coverage gives nonempty bounded actual data for bare `M`. -/
theorem r4HilbertMathlibSelfAdjointOperator_completed_hilbert_carrier_to_bare_M_bounded_actual_nonempty
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (hcover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorCompletedHilbertCarrierCoverageData I TR M)
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) :
    Nonempty (R4HilbertMathlibSelfAdjointOperatorBoundedActualData I TR M) := by
  exact ⟨r4HilbertMathlibSelfAdjointOperator_completed_hilbert_carrier_to_bare_M_bounded_actual_data I TR
    hBridge hcover M⟩

/-- Completed-Hilbert-carrier coverage gives a continuous top-domain representative. -/
theorem r4HilbertMathlibSelfAdjointOperator_completed_hilbert_carrier_to_bare_M_top_representative
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (hcover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorCompletedHilbertCarrierCoverageData I TR M)
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) :
