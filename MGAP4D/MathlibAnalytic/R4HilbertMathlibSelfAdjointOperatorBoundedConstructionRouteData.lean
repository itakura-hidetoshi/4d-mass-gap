...
/-! Bounded construction route data for the actual R4 mathlib self-adjoint operator. -/

/-- Construction-side route data sufficient to make the actual R4 mathlib operator
bounded.

The two required ingredients are deliberately separated:
* `fullDomainData` is the R4 construction-side proof that the actual operator has
  domain `⊤`.
* `closedGraphBridge` is the analytic closed-graph API turning a closed full-domain
  `LinearPMap` into a continuous representative.

Self-adjointness already supplies closedness through the existing actual toolkit. -/
structure R4HilbertMathlibSelfAdjointOperatorBoundedConstructionRouteData
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) where
  fullDomainData : R4HilbertMathlibSelfAdjointOperatorFullDomainData I TR M
  closedGraphBridge :
    r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR

/-- Extract full-domain data from the bounded construction route. -/
def r4HilbertMathlibSelfAdjointOperator_bounded_construction_route_full_domain_data
...
/-- Extract the closed-graph bridge from the bounded construction route. -/
theorem r4HilbertMathlibSelfAdjointOperator_bounded_construction_route_closed_graph_bridge
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR)
    (RD : R4HilbertMathlibSelfAdjointOperatorBoundedConstructionRouteData I TR M) :
    r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR :=
  RD.closedGraphBridge

/-- Bounded construction route data produces bounded actual data. -/
noncomputable def r4HilbertMathlibSelfAdjointOperator_bounded_construction_route_to_bounded_actual_data
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR)
    (RD : R4HilbertMathlibSelfAdjointOperatorBoundedConstructionRouteData I TR M) :
    R4HilbertMathlibSelfAdjointOperatorBoundedActualData I TR M :=
  r4HilbertMathlibSelfAdjointOperator_full_domain_data_to_bounded_actual_data I TR M
    RD.closedGraphBridge RD.fullDomainData

/-- Bounded construction route data yields the full bounded actual package. -/
theorem r4HilbertMathlibSelfAdjointOperator_bounded_construction_route_to_bounded_actual_package
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR)
    (RD : R4HilbertMathlibSelfAdjointOperatorBoundedConstructionRouteData I TR M) :
  ...
  letI : CompleteSpace
      (r4HilbertMathlibSelfAdjointCarrier I TR M.selfAdjointnessData) :=
    r4HilbertMathlibSelfAdjointCarrierCompleteSpace I TR M.selfAdjointnessData
  exact r4HilbertMathlibSelfAdjointOperator_full_domain_data_to_bounded_actual_package I TR M
    RD.closedGraphBridge RD.fullDomainData
