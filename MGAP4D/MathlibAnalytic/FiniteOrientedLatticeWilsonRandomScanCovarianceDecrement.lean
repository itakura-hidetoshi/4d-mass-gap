...
/-- Gibbs pairing is additive under subtraction in its right argument. -/
...
/-- Gibbs pairing is additive under subtraction in its left argument. -/
...
/-- A scalar may be pulled out of the right argument of Gibbs pairing. -/
...
/-- Gibbs pairing commutes with a finite sum in its right argument. -/
theorem finite_oriented_gibbsPairingReal_sum_right
    (L : FiniteOrientedLatticeWilsonSystem)
    {ι : Type*} [Fintype ι]
    (f : L.Configuration → ℝ)
...
theorem finite_oriented_randomScan_gibbsPairing_decrement_eq
  ...
  rw [finite_oriented_gibbsPairingReal_sum_right]
