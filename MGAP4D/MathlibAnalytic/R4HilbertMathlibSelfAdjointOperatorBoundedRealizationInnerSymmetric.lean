import MGAP4D.MathlibAnalytic.R4HilbertMathlibSelfAdjointOperatorBoundedRealizationAdjointEq
...
/-! Conditional inner-product symmetry for a bounded realization. -/
...
theorem r4HilbertMathlibSelfAdjointOperator_actual_bounded_realization_bounded_inner_symmetric
  ...
  calc
    inner ℝ (B x) y = inner ℝ (B.adjoint x) y := by
      exact (congrArg (fun T => inner ℝ (T x) y) hAdjEq).symm
    _ = inner ℝ x (B y) := by
