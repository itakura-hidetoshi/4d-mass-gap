...
continuous full-domain representative whose `toPMap` on `⊤` is definitionally
connected to that actual operator. Thus boundedness and full actual domain are
now part of the bare operator data itself, rather than being supplied by an
external route witness. This layer still does not invoke a spectral theorem and
does not state a spectral-gap result. -/
