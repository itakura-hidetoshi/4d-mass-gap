...
theorem continuous_compact_oriented_integrable_singleLinkHeatBathKernel_of_uniform_bound
    ...
    (M : ℝ) (hM : ∀ A, |f A| ≤ M) (A : C.base.Configuration) :
...
theorem continuous_compact_oriented_singleLinkHeatBathProjection_gibbsPairing_symm
    ...
    (Mf Mg : ℝ) (hMf0 : 0 ≤ Mf) (hMg0 : 0 ≤ Mg)
    (hMf : ∀ A, |f A| ≤ Mf) (hMg : ∀ A, |g A| ≤ Mg) :
  ...
  let Phi : C.base.Configuration × C.base.Configuration → ℝ :=
    fun z => f z.2 * g z.1
  let swapPhi : C.base.Configuration × C.base.Configuration → ℝ :=
    fun z => Phi (Prod.swap z)
  have hPhiSM : StronglyMeasurable Phi :=
    (hf.comp_measurable measurable_snd).mul (hg.comp_measurable measurable_fst)
  ...
  have hSwapSM : StronglyMeasurable swapPhi :=
    hPhiSM.comp_measurable measurable_swap
  have hSwapBound : ∀ z, |swapPhi z| ≤ Mf * Mg := by
    intro z
    exact hPhiBound (Prod.swap z)
  have hSwapInt : Integrable swapPhi J :=
    continuous_compact_oriented_integrable_joint_of_uniform_bound
      C target swapPhi hSwapSM (Mf * Mg) hSwapBound
  unfold ContinuousCompactOrientedGaugeWilsonSystem.gibbsPairingReal
  calc
    (∫ A, C.singleLinkHeatBathProjection target f A * g A ∂C.gibbsMeasure) =
      ∫ A, (∫ B, f B ∂C.singleLinkHeatBathKernel target A) * g A
        ∂C.gibbsMeasure := by
      apply integral_congr_ae
      filter_upwards [] with A
      rw [continuous_compact_oriented_integral_singleLinkHeatBathKernel_eq_projection C target f hf A]
    _ = ∫ A, ∫ B, f B * g A ∂C.singleLinkHeatBathKernel target A
        ∂C.gibbsMeasure := by
      apply integral_congr_ae
      filter_upwards [] with A
    ...
    _ = ∫ A, ∫ B, f A * g B ∂C.singleLinkHeatBathKernel target A
        ∂C.gibbsMeasure := by
      simpa [Phi, swapPhi] using
        (continuous_compact_oriented_integral_singleLinkHeatBathJointMeasure
          C target swapPhi hSwapInt)
    _ = ∫ A, f A * (∫ B, g B ∂C.singleLinkHeatBathKernel target A)
        ∂C.gibbsMeasure := by
      apply integral_congr_ae
      filter_upwards [] with A
      rw [integral_const_mul]
    _ = ∫ A, f A * C.singleLinkHeatBathProjection target g A
        ∂C.gibbsMeasure := by
      apply integral_congr_ae
      filter_upwards [] with A
      rw [continuous_compact_oriented_integral_singleLinkHeatBathKernel_eq_projection C target g hg A]
