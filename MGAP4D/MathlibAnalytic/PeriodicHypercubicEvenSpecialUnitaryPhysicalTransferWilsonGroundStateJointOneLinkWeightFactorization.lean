...
/-- The literal direct ground-state one-link fiber weight factors exactly
through the physical transfer normalization, the left and updated-right
nonnegative vacuum representatives, and the literal temporal-gauge Wilson
one-slab kernel.

This is only an exact unfolding of the already-canonical ground-state density.
It does not identify the normalized fiber with a regular conditional
distribution or with the raw Wilson one-link conditional law, and it asserts no
uniform quantitative bound. -/
theorem
