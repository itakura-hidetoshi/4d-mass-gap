import MGAP4D.MathlibAnalytic.R4HilbertMathlibSelfAdjointOperatorBoundedRealizationTopWitnessActualToolkitPackage
...
/-! Continuous witness package for conditional bounded realizations. -/

theorem r4HilbertMathlibSelfAdjointOperator_actual_exists_bounded_realization_continuous_witness_package
  ...
  letI : CompleteSpace
    ...
    r4HilbertMathlibSelfAdjointCarrierCompleteSpace I TR M.selfAdjointnessData
  intro hExists
  rcases r4HilbertMathlibSelfAdjointOperator_actual_exists_bounded_realization_top_witness_actual_toolkit_package
      I TR M hExists with
    ⟨hDomain, hDense, hClosed, hActualAdjEq, hActualSelfEqAdj, hActualAdjDomain,
      hActualAdjGraph, hGraphSelf, hGraphRecover, hFormal, hLeAdj, hAdjLe, hTopWitness⟩
  rcases hTopWitness with
    ⟨B, p, hp, hpTop, hBp, hTop, hDomainTopWitness, hBoundedAdjEq,
      hGraphTop, hGraphAdj, hMem, hAction, hActualInner, hInner⟩
  have hActualTop : M.mathlibOperator = B.toPMap ⊤ := hTop.symm
  have hAdjTop : B.adjoint.toPMap ⊤ = M.mathlibOperator := by
    rw [hBoundedAdjEq]
    exact hTop
  have hActualAdjTop : M.mathlibOperator = B.adjoint.toPMap ⊤ := hAdjTop.symm
  exact ⟨B, hTop, hActualTop, hAdjTop, hActualAdjTop, hBoundedAdjEq,
    hDomain, hDense, hClosed, hActualAdjEq, hActualSelfEqAdj, hActualAdjDomain,
