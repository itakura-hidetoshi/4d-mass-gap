...
theorem taylorPartialSum_realResolventJointMultilinearTraceCarrierJet_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint_directionFamily_sup
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in f, ∀ p, box.Contains p → ∀ z ∈ Z,
      continuousLinearMapJointMultilinearCarrierJetSupDistance (V := V) (W := ℝ)
        (continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierCompleteJetFromResolvent
          V m mixedOrder (H b) (continuousLinearMapRealResolvent
  ...
  let P : β → Fin (mixedOrder + 1) → Prop := fun b n => ∀ q ∈ I,
    ‖continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierFromResolvent
        V m n.1 (H b) (R b q) -
      continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierFromResolvent
        V m n.1 H0 (R0 q)‖ < epsilon
  have hn : ∀ n : Fin (mixedOrder + 1), ∀ᶠ b in f, P b n := by
    intro n
    have hPhi : Continuous (fun q :
      ...
      simpa [continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierFromResolvent] using
        continuous_continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierFromResolvent_directionFamily
          (continuousLinearMapTrace (V := V)) m n.1
