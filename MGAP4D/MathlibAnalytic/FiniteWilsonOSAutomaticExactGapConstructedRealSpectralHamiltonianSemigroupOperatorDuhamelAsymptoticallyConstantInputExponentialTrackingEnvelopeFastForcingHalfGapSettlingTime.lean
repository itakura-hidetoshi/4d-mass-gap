...
/-- Constructed finite Wilson left tracking has an explicit fast-forcing half-gap
settling time. -/
theorem finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_fastForcing_halfGap_left
  ...
  simpa [finiteWilsonConstructedHamiltonianLeftSteadyState,
    ...
    orthonormalDiagonalHamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_fastForcing_halfGap_left
      ...
      exactGapValueReal_pos hgapμ t₀ A F U F_lim C hC hF hFC hU0 hUdiag ε hε

/-- Constructed finite Wilson right tracking has the same explicit settling time. -/
theorem finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_fastForcing_halfGap_right
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D : FiniteWilsonOSAutomaticExactGapConstructedTransferOrbitContractionData W)
    (n : ℕ) (μ : ℝ) (hgapμ : exactGapValueReal ≤ μ) (t₀ : ℝ)
    (A : D.StateSpace →L[ℝ] D.StateSpace)
