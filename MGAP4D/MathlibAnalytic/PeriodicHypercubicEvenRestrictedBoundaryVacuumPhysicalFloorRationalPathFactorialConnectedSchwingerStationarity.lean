...
This is the higher-order connected statement: each partition block moment is
invariant, and the finite moment-cumulant polynomial is therefore invariant
term by term. -/
theorem
...
The two slots remain distinct even when `time 0 = time 1`, so different
observables may be inserted at one rational time without collapsing the
correlator. -/
def periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalTwoPointConnected
    (μ : Measure (ℚ → ℝ)) (time : Fin 2 → ℚ) (O : Fin 2 → ℝ → ℝ) : ℝ :=
  finiteTwoPointConnected
    (periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalFiniteSlotObservableMoment
...
/-- The same two-point connected function formed from translated moments. -/
def periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalTwoPointShiftedConnected
...
/-- Exact stationarity of the connected two-point function
`M₁₂ - M₁ M₂`. -/
theorem
    periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalFloorRationalPathFactorial_continuum_twoPointConnected_shift_eq_self
    (H : ℕ → ℕ)
    (N : ℕ) (hN : 0 < N)
    [Nontrivial (Matrix.specialUnitaryGroup (Fin N) ℂ)]
    (beta : ℕ → ℝ) (hbeta : ∀ n, 0 ≤ beta n)
    ...
    periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalTwoPointShiftedConnected
        L.continuumMeasure.toMeasure time O r =
      periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalTwoPointConnected
        L.continuumMeasure.toMeasure time O := by
  unfold periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalTwoPointShiftedConnected
  unfold periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalRationalTwoPointConnected
  apply finiteTwoPointConnected_congr
  · exact
      periodicHypercubicEvenRestrictedBoundaryVacuumPhysicalFloorRationalPathFactorial_continuum_finiteSlotObservableMoment_shift_eq_self
...
insertions.  This is an OS-facing Euclidean object; no Minkowski real-time
...
identity observable at every slot. -/
theorem
