...
/-- Exact normalized kernel equation at one fixed finest evaluation
configuration for the direct two-step comparison.  The finest/coarsest actual
op-norm normalization scalars remain distinct. -/
...
/-- Exact scalar-weighted orbit-fibre condition for the actual normalized
direct two-step transfer. -/
...
/-- Exact pointwise finite-sum formula for the independently operator-norm-
normalized direct two-step cross-volume residual. -/
...
/-- Vanishing of the actual normalized direct two-step residual is exactly the
named finite normalized kernel equation. -/
...
/-- At one fixed finest evaluation configuration, the normalized direct two-
step kernel equation is exactly the scalar-weighted coarse residual-gauge
orbit-fibre equation. -/
...
/-- Exact direct two-step criterion for actual operator-norm-normalized cross-
volume intertwining at arbitrary nonnegative coupling. -/
