...
/-- Compact-uniform convergence of a fixed symmetric multilinear resolvent
derivative after arbitrary finite-dimensional compression. -/
theorem iteratedDeriv_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniformOn_compact_product
    ...
    (J : V →L[ℝ] E) (Q : E →L[ℝ] V)
    (k symmetricOrder : ℕ) (H : Fin symmetricOrder → (V →L[ℝ] V))
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ a in l,
      ∀ lambda ∈ K, ∀ z ∈ Z,
        ‖continuousLinearMapRealResolventOperatorSymmetricDerivative symmetricOrder
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k (F a) lambda)) z H -
          continuousLinearMapRealResolventOperatorSymmetricDerivative symmetricOrder
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k S.limitResolvent lambda)) z H‖ < epsilon := by
  ...
  simpa [R, R0,
    continuousLinearMapRealResolventOperatorSymmetricDerivative] using ha'

/-- Simultaneous compact-uniform convergence on the finite rectangle of
Taylor derivative orders and symmetric multilinear derivative orders. -/
theorem iteratedDeriv_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniformOn_compact_product_rectangular
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    (S : ContinuousLinearMapOpenTaylorStrongLimitData l gap F)
    (B : ContinuousLinearMapOpenResolventNormBoundFamilyData gap F)
    (L : ContinuousLinearMapOpenResolventNormBoundData E)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ a in l,
      ∀ k : Fin (taylorOrder + 1), ∀ n : Fin (symmetricOrder + 1),
      ∀ lambda ∈ K, ∀ z ∈ Z,
        ‖continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k.1 (F a) lambda)) z (H n) -
          continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k.1 S.limitResolvent lambda)) z (H n)‖ < epsilon := by
  intro epsilon hepsilon
  let I := Fin (taylorOrder + 1) × Fin (symmetricOrder + 1)
  let P : α → I → Prop := fun a kn =>
    ∀ lambda ∈ K, ∀ z ∈ Z,
      ‖continuousLinearMapRealResolventOperatorSymmetricDerivative kn.2.1
          (continuousLinearMapCompression J Q
            (_root_.iteratedDeriv kn.1.1 (F a) lambda)) z (H kn.2) -
        continuousLinearMapRealResolventOperatorSymmetricDerivative kn.2.1
          (continuousLinearMapCompression J Q
            (_root_.iteratedDeriv kn.1.1 S.limitResolvent lambda)) z (H kn.2)‖ < epsilon
