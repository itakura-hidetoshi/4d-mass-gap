...
carriers.  The multilinear arity genuinely depends on the jet index. -/
...
/-- Complete joint Fréchet carrier jet generated by one compressed real
resolvent. -/
def continuousLinearMapJointSpectralOperatorRealResolventMultilinearCarrierJetFromResolvent
    ...
    ContinuousLinearMapJointMultilinearCarrierJet V (V →L[ℝ] V) m order :=
  fun n =>
    continuousLinearMapJointSpectralOperatorRealResolventMultilinearCarrierFromResolvent
      m n.1 H R

@[simp]
theorem continuousLinearMapJointSpectralOperatorRealResolventMultilinearCarrierJetFromResolvent_apply
    {V : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
    (m order : ℕ) (H : Fin m → (V →L[ℝ] V)) (R : V →L[ℝ] V)
    (n : Fin (order + 1)) :
...
resolvent. -/
def continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierJetFromResolvent
    {V W : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
    [NormedAddCommGroup W] [NormedSpace ℝ W]
...
@[simp]
theorem continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierJetFromResolvent_apply
    {V W : Type*} [NormedAddCommGroup V] [NormedSpace ℝ V]
    [NormedAddCommGroup W] [NormedSpace ℝ W]
    (φ : (V →L[ℝ] V) →L[ℝ] W) (m order : ℕ)
    (H : Fin m → (V →L[ℝ] V)) (R : V →L[ℝ] V)
    (n : Fin (order + 1)) :
    continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierJetFromResolvent
      φ m order H R n =
    continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierFromResolvent
      φ m n.1 H R :=
  rfl
...
resolvent. -/
def continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierJetSupFromResolvent
    (V : Type*) [NormedAddCommGroup V] [NormedSpace ℝ V]
...
@[simp]
theorem continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierJetSupFromResolvent_apply
    (V : Type*) [NormedAddCommGroup V] [NormedSpace ℝ V]
    [FiniteDimensional ℝ V]
    (m order : ℕ) (H : Fin m → (V →L[ℝ] V)) (R : V →L[ℝ] V)
    (n : Fin (order + 1)) :
    continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierJetSupFromResolvent
      V m order H R n =
...
family. -/
def continuousLinearMapJointSpectralOperatorRealResolventMultilinearTraceCarrierRectangularJetSupFromResolventFamily
    (V : Type*) [NormedAddCommGroup V] [NormedSpace ℝ V]
    [FiniteDimensional ℝ V]
    (m taylorOrder mixedOrder : ℕ) (H : Fin m → (V →L[ℝ] V))
    (R : Fin (taylorOrder + 1) → (V →L[ℝ] V)) :
