...
import MGAP4D.MathlibAnalytic.R4HilbertMathlibSelfAdjointOperatorCompletedRouteBoundedActualPackageEndpoints
...
/-! Route-backed accessors for bare actual R4 operator data. -/

/-- Generator-carrier coverage installs the central supply locally and returns the
field-like bounded actual accessor for bare `M`. -/
def r4HilbertMathlibSelfAdjointOperator_generator_carrier_accessor_bounded_actual_data
...
def r4HilbertMathlibSelfAdjointOperator_generator_carrier_accessor_full_domain_continuous_data
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (gcover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorGeneratorCarrierCoverageData I TR M)
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) :
...
/-- Generator-carrier coverage gives the field-like full-domain accessor theorem. -/
theorem r4HilbertMathlibSelfAdjointOperator_generator_carrier_accessor_actual_domain_eq_top
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
    (gcover : ∀ (M : R4HilbertMathlibSelfAdjointOperatorData I TR),
      R4HilbertMathlibSelfAdjointOperatorGeneratorCarrierCoverageData I TR M)
    (M : R4HilbertMathlibSelfAdjointOperatorData I TR) :
...
field-like bounded actual accessor for bare `M`. -/
def r4HilbertMathlibSelfAdjointOperator_quotient_carrier_accessor_bounded_actual_data
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
...
/-- Completed-pre-carrier coverage installs the central supply locally and returns
the field-like bounded actual accessor for bare `M`. -/
def r4HilbertMathlibSelfAdjointOperator_completed_pre_carrier_accessor_bounded_actual_data
    (hBridge : r4HilbertMathlibSelfAdjointOperatorClosedGraphFullDomainToContinuousRepresentativeBridge I TR)
