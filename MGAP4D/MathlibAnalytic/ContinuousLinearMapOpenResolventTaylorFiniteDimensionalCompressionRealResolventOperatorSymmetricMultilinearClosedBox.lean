...
/-- Uniform convergence of a fixed symmetric multilinear resolvent derivative
on a complete closed Taylor box for arbitrary joint time and Taylor-degree
nets. -/
theorem taylorPartialSum_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint
        ...
        ‖continuousLinearMapRealResolventOperatorSymmetricDerivative symmetricOrder
            (continuousLinearMapCompression J Q
              (continuousLinearMapTaylorPartialSum
                (F (a b)) p.center p.target (taylorDegree b))) z H -
          continuousLinearMapRealResolventOperatorSymmetricDerivative symmetricOrder
  ...
  have hsymmetric :=
    finiteDimensional_continuousObservable_tendsto_uniformOn
      R R0
      (fun T : V →L[ℝ] V =>
        continuousLinearMapRealResolventSymmetricDysonMultilinear
          symmetricOrder T H)
      (continuous_continuousLinearMapRealResolventSymmetricDysonMultilinear
        symmetricOrder H)
      M hM hR0 hR
  intro epsilon hepsilon
  have h := hsymmetric epsilon hepsilon
  filter_upwards [h] with b hb
  intro p hp z hz
  have hb' := hb (p, z) ⟨hp, hz⟩
  simpa [R, R0,
    continuousLinearMapRealResolventOperatorSymmetricDerivative] using hb'

/-- Simultaneous closed-box convergence of the full finite symmetric
multilinear derivative jet for arbitrary joint time and Taylor-degree nets. -/
theorem taylorPartialSum_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint_rectangular
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    (S : ContinuousLinearMapOpenTaylorStrongLimitData l gap F)
    (B : ContinuousLinearMapOpenResolventNormBoundFamilyData gap F)
    (L : ContinuousLinearMapOpenResolventNormBoundData E)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ b in m,
      ∀ n : Fin (symmetricOrder + 1), ∀ p, box.Contains p → ∀ z ∈ Z,
        ‖continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
            (continuousLinearMapCompression J Q
              (continuousLinearMapTaylorPartialSum
                (F (a b)) p.center p.target (taylorDegree b))) z (H n) -
          continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
            (continuousLinearMapCompression J Q (S.limitResolvent p.target)) z (H n)‖ < epsilon := by
  intro epsilon hepsilon
  let P : β → Fin (symmetricOrder + 1) → Prop := fun b n =>
    ∀ p, box.Contains p → ∀ z ∈ Z,
      ‖continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
          (continuousLinearMapCompression J Q
            (continuousLinearMapTaylorPartialSum
              (F (a b)) p.center p.target (taylorDegree b))) z (H n) -
        continuousLinearMapRealResolventOperatorSymmetricDerivative n.1
          (continuousLinearMapCompression J Q (S.limitResolvent p.target)) z (H n)‖ < epsilon
  have hn : ∀ n : Fin (symmetricOrder + 1), ∀ᶠ b in m, P b n := by
    intro n
    exact
      S.taylorPartialSum_realResolventOperatorSymmetricDerivative_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint
        B L hLgap hLresolvent J Q n.1 (H n) a taylorDegree ha hdegree
        box Z margin hmargin hlimitMargin M hM hlimitNorm epsilon hepsilon
  have hfinite : ∀ᶠ b in m, ∀ n : Fin (symmetricOrder + 1), P b n := by
    change {b | ∀ n : Fin (symmetricOrder + 1), P b n} ∈ m
    rw [show {b | ∀ n : Fin (symmetricOrder + 1), P b n} =
      ⋂ n ∈ (Finset.univ : Finset (Fin (symmetricOrder + 1))), {b | P b n} by
        ext b
        simp]
    exact (Filter.biInter_finset_mem
      (Finset.univ : Finset (Fin (symmetricOrder + 1)))).2
      (fun n _ => hn n)
