...
/-- On closed-box certificate data, contraction postcomposition cannot increase
the finite-response-family master order. -/
theorem JointRemainderClosedBoxSharpCertificateData.finiteResponseFamilyMasterOrder_postcompose_le
  ...
  simpa [JointRemainderClosedBoxSharpCertificateData.finiteResponseFamilyMasterOrder] using
    continuousLinearMapJointRemainderFiniteResponseFamilyMasterSafeOrder_postcompose_le
      ψ φs hψ C.hq0 C.hq1 C.hM hhalf

/-- The original closed-box finite-family master order controls every response
after contraction postcomposition over an arbitrary joint approximation net. -/
theorem JointRemainderClosedBoxSharpCertificateData.eventually_finiteResponseFamilyMaster_postcompose_norm_lt
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    {n : Filter β} {directions : ℕ}
    (C : JointRemainderClosedBoxSharpCertificateData
      (α := α) (β := β) (E := E) (V := V)
  ...
  intro p hp z hz
  refine ⟨hb p hp z hz, ?_, ht p hp z hz⟩
  intro φ hφ
  exact hr (ψ.comp φ)
    ((continuousLinearMapJointRemainder_mem_postcomposeResponseFamily_iff
...
/-- The closed-box binary-product master order packages one product observable,
its two coordinate observables, the carrier, and trace. -/
noncomputable def JointRemainderClosedBoxSharpCertificateData.binaryProductMasterOrder
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    {n : Filter β} {directions : ℕ}
    (C : JointRemainderClosedBoxSharpCertificateData
      (α := α) (β := β) (E := E) (V := V)
...
/-- The closed-box product response order lies below the binary-product master
order. -/
theorem JointRemainderClosedBoxSharpCertificateData.responseOrder_prod_le_binaryProductMasterOrder
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    {n : Filter β} {directions : ℕ}
    (C : JointRemainderClosedBoxSharpCertificateData
      (α := α) (β := β) (E := E) (V := V)
...
/-- The closed-box first-coordinate response order lies below the binary-product
master order. -/
theorem JointRemainderClosedBoxSharpCertificateData.responseOrder_left_le_binaryProductMasterOrder
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    {n : Filter β} {directions : ℕ}
    (C : JointRemainderClosedBoxSharpCertificateData
      (α := α) (β := β) (E := E) (V := V)
...
/-- The closed-box second-coordinate response order lies below the binary-product
master order. -/
theorem JointRemainderClosedBoxSharpCertificateData.responseOrder_right_le_binaryProductMasterOrder
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    {n : Filter β} {directions : ℕ}
    (C : JointRemainderClosedBoxSharpCertificateData
      (α := α) (β := β) (E := E) (V := V)
...
/-- The closed-box binary-product master order eventually controls the carrier,
...
theorem JointRemainderClosedBoxSharpCertificateData.eventually_binaryProductMaster_norm_lt
    {l : Filter α} {gap : ℝ} {F : α → ℝ → E →L[ℝ] E}
    {n : Filter β} {directions : ℕ}
    (C : JointRemainderClosedBoxSharpCertificateData
      (α := α) (β := β) (E := E) (V := V)
  ...
  have hright := C.eventually_response_norm_lt_of_order_le
    ...
    hepsilon
  filter_upwards [hfamily, hleft, hright] with b hf hl hr
  intro p hp z hz
  have hpack := hf p hp z hz
  exact ⟨hpack.1,
    hpack.2.1 (φ.prod θ) (by simp),
    hl p hp z hz,
    hr p hp z hz,
    hpack.2.2⟩
