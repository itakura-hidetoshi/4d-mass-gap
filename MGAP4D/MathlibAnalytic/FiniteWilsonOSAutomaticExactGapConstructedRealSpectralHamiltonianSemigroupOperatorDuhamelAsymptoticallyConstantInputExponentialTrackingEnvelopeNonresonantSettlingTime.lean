import MGAP4D.MathlibAnalytic.FiniteWilsonOSAutomaticExactGapConstructedRealSpectralHamiltonianSemigroupOperatorDuhamelAsymptoticallyConstantInputExponentialTrackingEnvelopeNonresonant
...
/-- Constructed finite Wilson non-resonant left tracking enters the target
accuracy after the explicit single-rate settling time. -/
theorem finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_nonresonant_left
  ...
  simpa [finiteWilsonConstructedHamiltonianLeftSteadyState,
    ...
    orthonormalDiagonalHamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_nonresonant_left
      ((D.hamiltonianSymmetric n).eigenvectorBasis D.stateFinrank)
      ((D.hamiltonianSymmetric n).eigenvalues D.stateFinrank)
      exactGapValueReal μ
      (D.hamiltonianEigenvalues_ge_exactGap n)
      exactGapValueReal_pos hμpos hgapμ t₀ A F U F_lim C hC hF hFC hU0 hUdiag ε hε

/-- Constructed finite Wilson non-resonant right tracking has the same explicit
single-rate settling time. -/
theorem finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_nonresonant_right
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D : FiniteWilsonOSAutomaticExactGapConstructedTransferOrbitContractionData W)
    (n : ℕ)
    (μ : ℝ)
  ...
  simpa [finiteWilsonConstructedHamiltonianRightSteadyState,
    ...
    orthonormalDiagonalHamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_norm_sub_le_epsilon_after_exponentialTrackingEnvelope_nonresonant_right
      ((D.hamiltonianSymmetric n).eigenvectorBasis D.stateFinrank)
      ((D.hamiltonianSymmetric n).eigenvalues D.stateFinrank)
      exactGapValueReal μ
      (D.hamiltonianEigenvalues_ge_exactGap n)
