...
/-- On `Ω⊥`, fast forcing gives left tracking at half the exact gap. -/
theorem finite_wilson_vacuum_orthogonal_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_exponentialTrackingEnvelope_fastForcing_halfGap_left
  ...
  exact
    finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_exponentialTrackingEnvelope_fastForcing_halfGap_left
      ...
      n μ hgapμ t₀ t ht A F U F_lim C hC hF hFC hU0 hU

/-- On `Ω⊥`, right tracking has the same fast-forcing half-gap estimate. -/
theorem finite_wilson_vacuum_orthogonal_real_spectral_hamiltonianSemigroup_operator_duhamel_asymptoticallyConstantInput_exponentialTrackingEnvelope_fastForcing_halfGap_right
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D : FiniteWilsonOSAutomaticExactGapVacuumOrthogonalCoerciveTransferOrbitContractionData W)
    (n : ℕ) (μ : ℝ) (hgapμ : exactGapValueReal ≤ μ)
    (t₀ t : ℝ) (ht : t₀ ≤ t)
