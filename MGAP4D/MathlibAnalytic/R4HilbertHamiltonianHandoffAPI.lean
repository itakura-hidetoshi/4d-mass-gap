...
/-- Handoff API for the R4 Hamiltonian input layer.

This exposes the Hamiltonian input carrier, domain, action, upstream generator
handoff, and readiness obligations. It still does not assert self-adjointness,
does not invoke a spectral theorem, and does not state a spectral-gap result. -/
...
theorem r4HilbertHamiltonianHandoff_constructed
    (M : R4HilbertHamiltonianInputData I TR) :
    (M.generatorData.semigroupData.osSemigroupReady ∧
      M.generatorData.generatorDomainDense ∧ M.generatorData.generatorGraphClosed ∧
        M.generatorData.generatorCompatibleWithSemigroup ∧
