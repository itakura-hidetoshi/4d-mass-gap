...
theorem reflection_fullTranslation
  ...
  have href := R.reflection_translate_neg_realizable n k X
  have href' :
      E.translate t (R.configurationReflection X) =
        R.configurationReflection (E.translate (-t) X) := by
    simpa [t] using
      (R.reflection_translate_neg_realizable n k
        (E.translate (-t) X)).symm
  rw [href']
  rw [E.translate_symm_apply_eq_neg]

/-- The theorem-generated finite state stationarity and reflection reversal imply
the realized Osterwalder--Schrader exchange identity.  This is not an
independent physical assumption. -/
theorem osBilinForm_positiveTranslation_exchange
  ...
  simpa only [map_mul, AlgEquiv.apply_symm_apply] using hstationary.symm
