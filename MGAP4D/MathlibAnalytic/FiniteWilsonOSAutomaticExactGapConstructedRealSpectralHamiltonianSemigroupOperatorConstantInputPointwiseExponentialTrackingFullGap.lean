...
/-- Constructed finite Wilson left tracking acts pointwise with the full exact-gap
    rate under exactly constant input. -/
theorem finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_constantInput_pointwise_exponentialTracking_fullGap_left
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D : FiniteWilsonOSAutomaticExactGapConstructedTransferOrbitContractionData W)
    (n : ℕ) (t₀ t : ℝ) (ht : t₀ ≤ t)
    (A : D.StateSpace →L[ℝ] D.StateSpace)
    (U : ℝ → (D.StateSpace →L[ℝ] D.StateSpace))
    (F_lim : D.StateSpace →L[ℝ] D.StateSpace) (y : D.StateSpace)
    (hU0 : U t₀ = A)
    (hU : ∀ r : ℝ,
      HasDerivAt U
  ...
  simpa [finiteWilsonConstructedHamiltonianLeftSteadyState,
    ...
    orthonormalDiagonalHamiltonianSemigroup_operator_constantInput_pointwise_exponentialTracking_fullGap_left
      ...
      exactGapValueReal_pos t₀ t ht A U F_lim y hU0 hUdiag

/-- Constructed finite Wilson right tracking has the identical pointwise full
    exact-gap estimate. -/
theorem finite_wilson_constructed_real_spectral_hamiltonianSemigroup_operator_constantInput_pointwise_exponentialTracking_fullGap_right
    {W : FiniteWilsonOSAutomaticApproximationFamily}
    (D : FiniteWilsonOSAutomaticExactGapConstructedTransferOrbitContractionData W)
    (n : ℕ) (t₀ t : ℝ) (ht : t₀ ≤ t)
    (A : D.StateSpace →L[ℝ] D.StateSpace)
