...
/-- The finite marginal can be computed in the symmetric interval obtained by
adding `d` to its canonical radius. -/
theorem linearMarkovIntegerCenteredFinitePathPMF_map_observeAt_add
    {Ω : Type*} [Fintype Ω]
  ...
  rw [← linearMarkovIntegerCenteredFinitePathPMF_map_restrictBy_of_detailedBalance
    initial transition hdb (linearMarkovIntegerFiniteSetRadius J) d]
  rw [PMF.map_comp]
  apply congrArg
    (fun f =>
      (linearMarkovIntegerCenteredFinitePathPMF initial transition
