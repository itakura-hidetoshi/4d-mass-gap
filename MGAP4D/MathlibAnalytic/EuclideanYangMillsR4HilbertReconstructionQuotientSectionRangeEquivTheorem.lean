import MGAP4D.MathlibAnalytic.EuclideanYangMillsR4HilbertReconstructionQuotientSectionRangeEquivModel
...
/-- Bundled outputs for the carrier-level equivalence between quotient classes and the selected section range. -/
def sectionRangeEquivOutputs
    ...
    (M : EuclideanYangMillsR4HilbertReconstructionQuotientSectionRangeEquivModel S K R4 A G H N F C I O Q P R U J V W X Y Z T) :
    Prop :=
  (∀ q, M.rangeToQuotient (M.quotientToRange q) = q) ∧
    (∀ x, M.quotientToRange (M.rangeToQuotient x) = x) ∧
...
/-- The section-range equivalence model proves its bundled outputs. -/
theorem sectionRangeEquivOutputs_holds
    {S : EuclideanYangMillsContinuumMeasureConstructionSpine}
    {K : EuclideanYangMillsCompleteConstructionClosure S}
    {R4 : EuclideanYangMillsR4GaugeFieldConstructionClosure S K}
    {A : EuclideanYangMillsR4GaugeActionClosure S K R4}
    ...
    {T : EuclideanYangMillsR4HilbertReconstructionQuotientSectionRepresentativeProjectionClosure S K R4 A G H N F C I O Q P R U J V W X Y Z}
    (M : EuclideanYangMillsR4HilbertReconstructionQuotientSectionRangeEquivModel S K R4 A G H N F C I O Q P R U J V W X Y Z T) :
    M.sectionRangeEquivOutputs :=
  ⟨M.rightInverse,
    M.leftInverse,
    M.sectionRepresentativeProjection,
    M.reflectionPositive,
    M.euclideanInvariant,
    M.gaugeInvariant⟩

/-- The representative projection closure induces a carrier-level selected section-range equivalence model. -/
theorem nonempty_ofSectionRepresentativeProjectionClosure
    (S : EuclideanYangMillsContinuumMeasureConstructionSpine)
    (K : EuclideanYangMillsCompleteConstructionClosure S)
    (R4 : EuclideanYangMillsR4GaugeFieldConstructionClosure S K)
    (A : EuclideanYangMillsR4GaugeActionClosure S K R4)
    ...
    (Z : EuclideanYangMillsR4HilbertReconstructionQuotientSectionRepresentativeStableClosure S K R4 A G H N F C I O Q P R U J V W X Y)
    (T : EuclideanYangMillsR4HilbertReconstructionQuotientSectionRepresentativeProjectionClosure S K R4 A G H N F C I O Q P R U J V W X Y Z) :
    Nonempty (EuclideanYangMillsR4HilbertReconstructionQuotientSectionRangeEquivModel S K R4 A G H N F C I O Q P R U J V W X Y Z T) :=
  ⟨ofSectionRepresentativeProjectionClosure S K R4 A G H N F C I O Q P R U J V W X Y Z T⟩
