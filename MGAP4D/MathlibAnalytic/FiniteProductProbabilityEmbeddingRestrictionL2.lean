...
embedding, then reindexes that subtype back to the source index through
`Embedding.equivRange`.  Both steps are Mathlib measure-preserving maps. -/
theorem measurePreserving_finiteProductProbabilityEmbeddingRestriction
  ...
  have hRestrict :
    ...
    measurePreserving_finiteProductProbabilityRestriction
      (fun _ : ι => μ) p
  let erange : κ ≃ Subtype p := e.equivRange
  have hReindex :
      MeasurePreserving
  ...
  convert h using 1
  funext x k
  simp [Function.comp_def, erange, p]
