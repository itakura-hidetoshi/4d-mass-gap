...
/-- Diagonal no-rate convergence of trace interpolants and exact trace
remainders on a complete closed Taylor box. -/
theorem taylorPartialSum_realResolventNewtonHermiteTracePair_finiteDimensionalCompression_tendsto_uniform_closedBox_of_tendsto_degree
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ a in l, ∀ p, box.Contains p → ∀ q ∈ T,
      ‖continuousLinearMapRealResolventTracePair (continuousLinearMapRealResolventNewtonHermitePair interpolationDegree (continuousLinearMapCompression J Q (continuousLinearMapTaylorPartialSum (F a) p.center p.target (degree a))) (nodes q) (eval q)) - continuousLinearMapRealResolventTracePair (continuousLinearMapRealResolventNewtonHermitePair interpolationDegree (continuousLinearMapCompression J Q (S.limitResolvent p.target)) (nodes q) (eval q))‖ < epsilon := by
  exact
    S.taylorPartialSum_realResolventNewtonHermiteTracePair_finiteDimensionalCompression_tendsto_uniform_closedBox_of_joint
      B L hLgap hLresolvent J Q interpolationDegree
      (fun a => a) degree tendsto_id hdegree box nodes eval T Z
      hnodes heval D hD hdist margin hmargin hlimitMargin M hM hlimitNorm
