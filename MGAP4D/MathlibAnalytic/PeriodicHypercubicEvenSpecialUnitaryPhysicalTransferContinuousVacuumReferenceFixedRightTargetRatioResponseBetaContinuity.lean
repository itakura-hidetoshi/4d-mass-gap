...
First transport the existing normalized kernel law along an arbitrary continuous
parameter map, allowing the observable to depend on extra test variables. Then
...
mass floor, or continuity assumption on the response profile is introduced. The
canonical supremum profile is not redefined. All local instances are explicitly
named to keep the combined import closure free of autogenerated name collisions.
-/
  ...
  exact realIntegralWeightedProbabilityMeasure_integral_continuous
    ...
    (fun p A => (mul_pos
      (periodicHypercubicEvenSpecialUnitaryPhysicalOneSlabContinuousVacuumRepresentative_pos
        H N hN (beta p).1 (beta p).2 A)
      (periodicHypercubicEvenSpecialUnitaryTemporalGaugeOneSlabKernel_pos
        H N (beta p).1 A (C p))).le)
    (fun p => fixedRightResponse_continuousWeight_integral_pos
      H N hN (beta p).1 (beta p).2 (C p))
...
private theorem fixedRightResponse_update_continuous
  ...
  by_cases hi : i = e
  · subst i
    simpa only [Function.update_same] using hg
  · simpa only [Function.update_noteq hi] using (continuous_apply i).comp hB
