...
/-- Canonical OS compact-uniform complete Banach-valued response-jet stability
when the complete finite perturbation family moves with defect time. -/
theorem VacuumSemigroupGapSlope.canonicalJointMultilinearResponseCarrierRectangularJet_tendsto_uniformOn_compact_directionFamily_sup
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ tau in G.admissibleRescaledDefectTimeFilter, ∀ lambda ∈ K, ∀ z ∈ Z,
      continuousLinearMapJointMultilinearCarrierRectangularJetSupDistance
        (continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierRectangularJetFromResolventFamily φ m taylorOrder mixedOrder (H tau) (continuousLinearMapCompressedIteratedDerivRealResolventFamily J Q taylorOrder (G.admissibleRescaledDefectTaylorResolvent T hInnerSymmetric tau) lambda z))
        (continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierRectangularJetFromResolventFamily φ m taylorOrder mixedOrder H0 (continuousLinearMapCompressedIteratedDerivRealResolventFamily J Q taylorOrder (G.vacuumOrthogonalContinuumTaylorResolvent T hP hInnerSymmetric hSelf) lambda z)) < epsilon := by
  exact
    (G.canonicalRescaledDefectTaylorStrongLimitData T hP hInnerSymmetric hSelf).iteratedDeriv_realResolventJointMultilinearResponseCarrierRectangularJet_finiteDimensionalCompression_tendsto_uniformOn_compact_product_directionFamily_sup
