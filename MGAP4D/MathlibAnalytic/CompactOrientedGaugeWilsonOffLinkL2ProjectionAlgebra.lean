...
theorem continuous_compact_oriented_singleLinkHeatBathProjectionL2_idempotent
  ...
  letI : Fact (m ≤ (inferInstance : MeasurableSpace C.base.Configuration)) :=
    ⟨hm⟩
  let q : lpMeas ℝ ℝ m 2 C.gibbsMeasure :=
    condExpL2 ℝ ℝ hm f
  change
    ((condExpL2 ℝ ℝ hm (q : Lp ℝ 2 C.gibbsMeasure) :
        lpMeas ℝ ℝ m 2 C.gibbsMeasure) :
      Lp ℝ 2 C.gibbsMeasure) =
        (q : Lp ℝ 2 C.gibbsMeasure)
  have hq :
      (condExpL2 ℝ ℝ hm (q : Lp ℝ 2 C.gibbsMeasure) :
        lpMeas ℝ ℝ m 2 C.gibbsMeasure) = q := by
    unfold condExpL2
    exact Submodule.orthogonalProjectionOnto_mem_subspace_eq_self q
  exact congrArg
    (fun x : lpMeas ℝ ℝ m 2 C.gibbsMeasure =>
