...
/-- Real-valued count of one direct finest-to-coarsest configuration fibre. -/
...
/-- The direct finest fibre first variation changes by direct kernel
multiplicity and the constant two-step embedding scale times the finest
spatial-action defect. -/
theorem finiteEvenFourTorusZ2GaugeInvariantTwoStepFineConfigurationFiberKernelCoefficientFirstVariation_sub
  ...
  have hpoint : ∀ B : FiniteEvenFourTorusZ2SliceConfiguration
    ...
    · simp [hB]
  have hcount :=
    finiteEvenFourTorusZ2SliceConfigurationTwoStepCoarseMap_fiberIndicatorSum H b
...
/-- The direct coarsest first variation changes by the constant two-step scale
times the coarsest spatial-action defect. -/
...
/-- At beta zero the direct finest analytic coefficient is direct-kernel
multiplicity times the constant two-step embedding scale. -/
  ...
  calc
    (∑ B : FiniteEvenFourTorusZ2SliceConfiguration
      ...
      if finiteEvenFourTorusZ2SliceConfigurationCoarseMap H
          (finiteEvenFourTorusZ2SliceConfigurationCoarseMap
            (finiteEvenFourTorusDoubleRefinement H) B) = b then
        1 * finiteEvenFourTorusZ2GaugeInvariantTwoStepCoarseEmbeddingCardinalityScale H
      else 0) =
      finiteEvenFourTorusZ2GaugeInvariantTwoStepCoarseEmbeddingCardinalityScale H *
        (∑ B : FiniteEvenFourTorusZ2SliceConfiguration
            (finiteEvenFourTorusDoubleRefinement
...
/-- At beta zero the direct coarsest analytic coefficient is the constant
two-step embedding scale. -/
...
/-- Identity anchoring reduces the whole direct beta-zero projective first
variation to one finest-minus-coarsest spatial Wilson action defect. -/
theorem finiteEvenFourTorusZ2GaugeInvariantTwoStepGlobalProjectiveConfigurationFiberObstructionFirstVariation_identityAnchor_eq_spatialDefect
  ...
  have hCoarse :
    ...
    simpa using finiteEvenFourTorusZ2GaugeInvariantTwoStepCoarseKernelWeightFirstVariation_sub
      H energyIdentity energyNontrivial A 1 (1 : FiniteEvenFourTorusZ2SliceConfiguration H)
  have hFineZeroA :=
    finiteEvenFourTorusZ2GaugeInvariantTwoStepFineConfigurationFiberKernelCoefficientAnalytic_zero
      H energyIdentity energyNontrivial A (1 : FiniteEvenFourTorusZ2SliceConfiguration H)
  have hFineZeroOne :=
    finiteEvenFourTorusZ2GaugeInvariantTwoStepFineConfigurationFiberKernelCoefficientAnalytic_zero
      H energyIdentity energyNontrivial
        ...
        (1 : FiniteEvenFourTorusZ2SliceConfiguration H)
  have hCoarseZeroA :=
    finiteEvenFourTorusZ2GaugeInvariantTwoStepCoarseKernelWeightAnalytic_zero
      H energyIdentity energyNontrivial A (1 : FiniteEvenFourTorusZ2SliceConfiguration H)
  have hCoarseZeroOne :=
    finiteEvenFourTorusZ2GaugeInvariantTwoStepCoarseKernelWeightAnalytic_zero
      H energyIdentity energyNontrivial
...
/-- If the finest configuration lies in the direct two-step coarse-map kernel,
only its finest spatial Wilson action defect remains. -/
