...
theorem iteratedDeriv_realResolventFiniteParameterMixedResponse_tendsto_uniformOn_compact
  ...
  simpa only [
    continuousLinearMapFiniteParameterRealResolventSymmetricDerivativeLinearResponse_apply] using
    continuousLinearMap_map_sub_norm_lt_of_norm_sub_lt φ hepsilon
      (ha lambda hlambda z hz)
...
theorem iteratedDeriv_realResolventFiniteParameterTaylorResponse_tendsto_uniformOn_compact
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ a in l,
      ∀ lambda ∈ K, ∀ z ∈ Z,
        ‖continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse
            ...
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k (F a) lambda)) H z 0 h -
          continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse
            φ parameterOrder parameterDimension
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k S.limitResolvent lambda)) H z 0 h‖ < epsilon := by
  intro epsilon hepsilon
  have hdelta : 0 < epsilon / (‖φ‖ + 1) := div_pos hepsilon (by positivity)
  filter_upwards [
  ...
  intro lambda hlambda z hz
  simpa only [
    continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse_apply] using
    continuousLinearMap_map_sub_norm_lt_of_norm_sub_lt φ hepsilon
      (ha lambda hlambda z hz)
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ a in l,
      ...
      ∀ lambda ∈ K, ∀ z ∈ Z,
        ‖continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse
            ...
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k.1 (F a) lambda)) H z 0 h -
          continuousLinearMapFiniteParameterRealResolventTaylorDysonResponse
            φ n.1 parameterDimension
            (continuousLinearMapCompression J Q
              (_root_.iteratedDeriv k.1 S.limitResolvent lambda)) H z 0 h‖ < epsilon := by
  intro epsilon hepsilon
  have hdelta : 0 < epsilon / (‖φ‖ + 1) := div_pos hepsilon (by positivity)
  filter_upwards [
