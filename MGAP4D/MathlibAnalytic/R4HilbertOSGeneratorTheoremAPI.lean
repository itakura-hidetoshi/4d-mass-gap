...
theorem r4HilbertOSGeneratorTheorem_semigroup_ready
    (M : R4HilbertOSGeneratorInputData I TR) :
    M.semigroupData.osSemigroupReady :=
  r4HilbertCompletedOSSemigroupHandoff_ready I TR M.semigroupData
...
/-- The theorem API for the R4 OS generator input layer.

This theorem exposes the semigroup readiness together with the generator graph
input obligations. It still does not identify the generator as a physical
Hamiltonian, does not assert self-adjointness, and does not introduce spectral
or spectral-gap claims. -/
