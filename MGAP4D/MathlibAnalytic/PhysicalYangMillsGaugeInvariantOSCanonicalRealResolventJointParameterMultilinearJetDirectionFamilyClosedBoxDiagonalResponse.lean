...
/-- Canonical OS diagonal no-rate Banach-valued response-jet convergence when
Taylor degree and the complete perturbation family move with defect time. -/
theorem VacuumSemigroupGapSlope.canonicalJointMultilinearResponseCarrierJet_tendsto_uniform_closedBox_of_tendsto_degree_directionFamily_sup
    ...
    ∀ epsilon : ℝ, 0 < epsilon → ∀ᶠ tau in G.admissibleRescaledDefectTimeFilter,
      ...
      continuousLinearMapJointMultilinearCarrierJetSupDistance (V := V) (W := W)
        (continuousLinearMapJointSpectralOperatorRealResolventMultilinearResponseCarrierCompleteJetFromResolvent
          φ m mixedOrder (H tau) (continuousLinearMapCompressedTaylorPartialSumRealResolventAt J Q
  ...
  exact G.canonicalJointMultilinearResponseCarrierJet_tendsto_uniform_closedBox_of_joint_directionFamily_sup
