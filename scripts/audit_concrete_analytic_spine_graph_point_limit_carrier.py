...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineGraphPointLimitCarrier.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")

TARGET_ANCHORS = (
    "ConcreteGraphPointLimitCarrierSurface",
    ...
    "concreteIdentityGraphPointLimitCarrierSurface",
    "concrete_identity_graph_point_limit_carrier_boundary",
    "concreteAnalyticSpineR2GraphPointLimitCarrierSurfaceReady",
    "concrete_analytic_spine_r2_graph_point_limit_carrier_surface_ready",
    "concreteAnalyticSpineR2GraphPointLimitCarrierHardResidualBoundaryHeld",
)
...
def main() -> None:
    ...
    print("Concrete analytic spine graph point limit carrier audit")
    ...
    if failures:
        print("Concrete analytic spine graph point limit carrier audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine graph point limit carrier audit passed")
