...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineGraphNormConvergentSequence.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")

TARGET_ANCHORS = (
    "ConcreteGraphNormConvergentSequenceSurface",
    "graphNormLimit : ℝ",
    "graphNormLimitWitness :",
    "convergentSequenceBoundaryNotCompletionTheorem : Prop",
    "concrete_identity_graph_norm_sequence_converges_zero",
    "concreteIdentityGraphNormConvergentSequenceSurface",
    "concrete_identity_graph_norm_convergent_sequence_boundary",
    "concreteAnalyticSpineR2GraphNormConvergentSequenceSurfaceReady",
    "concrete_analytic_spine_r2_graph_norm_convergent_sequence_surface_ready",
    "concreteAnalyticSpineR2GraphNormConvergentSequenceHardResidualBoundaryHeld",
)
...
def main() -> None:
    ...
    print("Concrete analytic spine graph norm convergent sequence audit")
    ...
    if failures:
        print("Concrete analytic spine graph norm convergent sequence audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine graph norm convergent sequence audit passed")
