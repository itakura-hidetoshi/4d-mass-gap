...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineOperatorLane.lean")
...
TARGET_ANCHORS = (
    ...
    "concrete_analytic_spine_operator_lane_bootstrap_surface_ready",
    "concreteAnalyticSpineOperatorLaneBootstrapHardResidualBoundaryHeld",
    "concrete_analytic_spine_operator_lane_bootstrap_hard_residual_boundary_held",
)
...
ROOT_ANCHOR = "import MGAP4D.MathlibAnalytic.ConcreteAnalyticSpineOperatorLane"
...
def main() -> None:
    ...
    print("Concrete analytic spine operator lane audit")
    ...
    if failures:
        print("Concrete analytic spine operator lane audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine operator lane audit passed")
