#!/usr/bin/env python3
from pathlib import Path
import re
import sys

TOKEN_RE = re.compile(r"\b(sorry|admit|axiom|constant)\b")
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineL2DiagonalWeightThreshold.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")
ANCHORS = [
    "concrete_l2_diagonal_weight_exceeds_nat",
    "concrete_l2_diagonal_weight_exceeds_every_nat_threshold",
    "concreteL2DiagonalWeightThresholdWitness",
    "concrete_l2_diagonal_weight_threshold_witness_spec",
    "ConcreteL2DiagonalWeightThresholdSurface",
    "concreteL2DiagonalWeightThresholdSurface",
    "concrete_analytic_spine_l2_diagonal_weight_threshold_surface_ready",
    "concrete_analytic_spine_l2_diagonal_weight_threshold_hard_residual_boundary_held",
]
ROOT_ANCHOR = "import MGAP4D.MathlibAnalytic.ConcreteAnalyticSpineL2DiagonalWeightThreshold"


def main():
    failures = []
    target = TARGET.read_text(encoding="utf-8") if TARGET.exists() else ""
    root = ROOT.read_text(encoding="utf-8") if ROOT.exists() else ""
    for i, line in enumerate(target.splitlines(), start=1):
        if TOKEN_RE.search(line):
            failures.append(f"{TARGET}:{i}: forbidden Lean token")
    for anchor in ANCHORS:
        if anchor not in target:
            failures.append(f"missing anchor {anchor!r}")
    if ROOT_ANCHOR not in root:
        failures.append("missing root import anchor")
    print("Concrete analytic spine l2 diagonal threshold audit")
    print(f"Anchors audited: {len(ANCHORS)}")
    print("Root imports audited: 1")
    if failures:
        print("Concrete analytic spine l2 diagonal threshold audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine l2 diagonal threshold audit passed")


if __name__ == "__main__":
    main()
