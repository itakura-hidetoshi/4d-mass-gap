...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineGraphNormBoundedSequence.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")

TARGET_ANCHORS = (
    "ConcreteGraphNormBoundedSequenceSurface",
    "graphNormBound : ℝ",
    "graphNormBoundWitness :",
    "boundedSequenceBoundaryNotCompletionTheorem : Prop",
    "concrete_identity_graph_norm_sequence_bound_zero",
    "concreteIdentityGraphNormBoundedSequenceSurface",
    "concreteAnalyticSpineR2GraphNormBoundedSequenceSurfaceReady",
    "concrete_analytic_spine_r2_graph_norm_bounded_sequence_surface_ready",
    "concreteAnalyticSpineR2GraphNormBoundedSequenceHardResidualBoundaryHeld",
)
...
def main() -> None:
    ...
    print("Concrete analytic spine graph norm bounded sequence audit")
    ...
    if failures:
        print("Concrete analytic spine graph norm bounded sequence audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine graph norm bounded sequence audit passed")
