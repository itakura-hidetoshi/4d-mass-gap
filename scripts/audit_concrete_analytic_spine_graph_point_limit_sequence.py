...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineGraphPointLimitSequence.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")

TARGET_ANCHORS = (
    "ConcreteGraphPointLimitSequenceSurface",
    "graphPointLimitLaw :",
    "graphPointLimitBoundaryNotClosureTheorem : Prop",
    "concrete_identity_graph_sequence_point_eq_zero",
    "concreteIdentityGraphPointLimitSequenceSurface",
    "concrete_identity_graph_point_limit_sequence_boundary",
    "concreteAnalyticSpineR2GraphPointLimitSequenceSurfaceReady",
    "concrete_analytic_spine_r2_graph_point_limit_sequence_surface_ready",
    "concreteAnalyticSpineR2GraphPointLimitSequenceHardResidualBoundaryHeld",
)
...
def main() -> None:
    ...
    print("Concrete analytic spine graph point limit sequence audit")
    ...
    if failures:
        print("Concrete analytic spine graph point limit sequence audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine graph point limit sequence audit passed")
