...
VERSION = "mgap4d_kuuos_ci_completion_source_inbox_v0_2"
READY = "MGAP4D_KUUOS_CI_SOURCE_INBOX_READY"
BLOCKED = "MGAP4D_KUUOS_CI_SOURCE_INBOX_BLOCKED"
ACK_READY = "MGAP4D_KUUOS_CI_SOURCE_INBOX_ACK_READY"
ACK_BLOCKED = "MGAP4D_KUUOS_CI_SOURCE_INBOX_ACK_BLOCKED"
...
PENDING_TITLE_PREFIX = "[KuuOS MCP CI Pending]"
...
def compile_source_inbox(sender_result: Mapping[str, Any]) -> dict[str, Any]:
    blockers: list[str] = []
    dispatch = _m(sender_result.get("dispatch"))
    payload = _m(dispatch.get("client_payload"))

    if sender_result.get("dispatch_allowed") is not True:
        blockers.append("sender_dispatch_not_admitted")
    if str(sender_result.get("status", "")) != "MGAP4D_KUUOS_CI_COMPLETION_SENDER_READY":
        blockers.append("sender_status_not_ready")
    if str(dispatch.get("event_type", "")) != "kuuos_ci_completion_v1_1":
        blockers.append("dispatch_event_type_mismatch")
    ...
    record = {
        ...
        "mcp_reobserve_request": {
            ...
            "expected_workflow_name": SOURCE_WORKFLOW,
        ...
        "optional_kuuos_mirror": {
            "repository": DESTINATION_REPOSITORY,
            "event_type": "kuuos_ci_completion_v1_1",
            "cross_repository_credential_required": True,
        },
        "boundary": {
            "source_issue_is_wakeup_signal_only": True,
            "source_issue_is_not_ci_success_evidence": True,
            "same_repository_persistence_requires_no_cross_repo_secret": True,
        ...
        "sender_result_digest": _digest(sender_result),
    ...
    if not blockers:
        issue_spec = {
            ...
            "dedupe_query": f'repo:{SOURCE_REPOSITORY} is:issue in:title "{issue_key}"',
        }
...
def compile_source_ack(
    ...
    if str(record.get("issue_key", "")) != _identity_key(identity):
    ...
    for field, agrees in exact.items():
        if not agrees:
...
def _sender_fixture(*, conclusion: str = "success") -> dict[str, Any]:
    sha = "a" * 40
    payload = {
    ...
    dispatch = {"event_type": "kuuos_ci_completion_v1_1", "client_payload": payload}
    return {
        "version": SENDER_VERSION,
...
def _verification_fixture(record: Mapping[str, Any], *, route: str = "verified_success") -> dict[str, Any]:
    identity = _m(record.get("identity"))
    return {
...
def self_check() -> None:
    ready = compile_source_inbox(_sender_fixture())
    assert ready["status"] == READY
    assert ready["persist_allowed"] is True
    assert ready["issue_spec"]["title"].startswith(PENDING_TITLE_PREFIX)
    assert ready["record"]["boundary"]["same_repository_persistence_requires_no_cross_repo_secret"] is True

    failed = compile_source_inbox(_sender_fixture(conclusion="failure"))
    assert failed["persist_allowed"] is True
    assert failed["record"]["source"]["conclusion"] == "failure"

    wrong_base_sender = _sender_fixture()
    wrong_base_sender["dispatch"]["client_payload"]["pr_base_branch"] = "main"
    wrong_base = compile_source_inbox(wrong_base_sender)
    assert wrong_base["persist_allowed"] is False
    assert "canonical_base_mismatch" in wrong_base["blockers"]

    no_fresh_sender = _sender_fixture()
    no_fresh_sender["dispatch"]["client_payload"]["fresh_mcp_reobservation_required"] = False
    no_fresh = compile_source_inbox(no_fresh_sender)
    assert no_fresh["persist_allowed"] is False
    assert "fresh_mcp_reobservation_boundary_missing" in no_fresh["blockers"]

    record = ready["record"]
    verification = _verification_fixture(record)
    ack = compile_source_ack(record, verification)
    assert ack["status"] == ACK_READY
    assert ack["close_issue_allowed"] is True
    assert ack["ack"]["merge_authority_granted"] is False

    unverified = dict(verification)
    unverified["status"] = "KUUOS_GITHUB_CI_COMPLETION_REENTRY_VERIFY_BLOCKED"
    blocked = compile_source_ack(record, unverified)
    assert blocked["close_issue_allowed"] is False
    assert "fresh_mcp_verification_not_verified" in blocked["blockers"]

    wrong_head = dict(verification)
    wrong_head["head_sha"] = "b" * 40
    mismatch = compile_source_ack(record, wrong_head)
    assert mismatch["close_issue_allowed"] is False
    assert "verification_head_sha_mismatch" in mismatch["blockers"]


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--self-check", action="store_true")
    parser.add_argument("--sender-result")
    parser.add_argument("--output-dir", default=".kuuos/ci-completion-source-inbox-v0-2")
    ...
    if args.self_check:
        self_check()
        print("MGAP4D_KUUOS_CI_COMPLETION_SOURCE_INBOX_V0_2_SELF_CHECK_OK")
        return 0
    ...
    if args.ack_record or args.verification:
        ...
        result = compile_source_ack(record, verification)
        _write(out / "source_inbox_ack_spec.json", result)
        print(result["status"])
        return 0 if result["close_issue_allowed"] else 2

    if not args.sender_result:
        parser.error("--sender-result is required unless --self-check or ack mode is used")
    sender_result = json.loads(pathlib.Path(args.sender_result).read_text(encoding="utf-8"))
    if not isinstance(sender_result, Mapping):
        raise SystemExit("sender result must be a JSON object")
    result = compile_source_inbox(sender_result)
    _write(out / "source_inbox_result.json", result)
    if result["persist_allowed"]:
        _write(out / "source_inbox_record.json", result["record"])
        _write(out / "source_inbox_issue_spec.json", result["issue_spec"])
    print(result["status"])
