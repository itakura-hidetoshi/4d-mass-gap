...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineL2R2ProgressIndex.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")
ANCHORS = [
    "ConcreteL2R2ProgressIndexSurface",
    "concreteL2R2ProgressIndexSurface",
    "concreteAnalyticSpineL2R2ProgressIndexSurfaceReady",
    "concrete_analytic_spine_l2_r2_progress_index_surface_ready",
    "concreteAnalyticSpineL2R2ProgressIndexHardResidualBoundaryHeld",
    "concrete_analytic_spine_l2_r2_progress_index_hard_residual_boundary_held",
...
ROOT_ANCHOR = "import MGAP4D.MathlibAnalytic.ConcreteAnalyticSpineL2R2ProgressIndex"
...
def main():
    ...
    print("Concrete analytic spine l2 R2 progress index audit")
    print(f"Anchors audited: {len(ANCHORS)}")
    print("Root imports audited: 1")
    if failures:
        print("Concrete analytic spine l2 R2 progress index audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine l2 R2 progress index audit passed")
