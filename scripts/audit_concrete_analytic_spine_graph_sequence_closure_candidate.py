...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineGraphSequenceClosureCandidate.lean")
ROOT = Path("MGAP4D/MathlibAnalytic.lean")

TARGET_ANCHORS = (
    "ConcreteGraphSequenceClosureCandidateSurface",
    ...
    "concrete_analytic_spine_r2_graph_sequence_closure_candidate_surface_ready",
    "concreteAnalyticSpineR2GraphSequenceClosureCandidateHardResidualBoundaryHeld",
)
...
def main() -> None:
    ...
    print("Concrete analytic spine graph sequence closure candidate audit")
    ...
    if failures:
        print("Concrete analytic spine graph sequence closure candidate audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine graph sequence closure candidate audit passed")
