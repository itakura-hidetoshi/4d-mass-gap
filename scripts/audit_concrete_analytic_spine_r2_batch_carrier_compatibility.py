...
TARGET = Path("MGAP4D/MathlibAnalytic/ConcreteAnalyticSpineR2BatchCarrierCompatibility.lean")
...
TARGET_ANCHORS = (
    ...
    "concreteAnalyticSpineR2BatchCarrierCompatibilityHardResidualBoundaryHeld",
    "concrete_analytic_spine_r2_batch_carrier_compatibility_hard_residual_boundary_held",
)
...
def main() -> None:
    ...
    print("Concrete analytic spine R2 batch carrier compatibility audit")
    ...
    if failures:
        print("Concrete analytic spine R2 batch carrier compatibility audit failed:", file=sys.stderr)
        for f in failures:
            print(f"  {f}", file=sys.stderr)
        raise SystemExit(1)
    print("Concrete analytic spine R2 batch carrier compatibility audit passed")
